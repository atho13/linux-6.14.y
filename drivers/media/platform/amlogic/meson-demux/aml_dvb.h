...
#define SEC_CNT_MAX (3)
	...
	irq_handler_t thread_irq_handler;
};
...
enum { AM_TS_DISABLE, AM_TS_PARALLEL, AM_TS_SERIAL };
...
extern int aml_dsc_hw_set_source(struct aml_dsc *dsc, dmx_source_t src,
				 dmx_source_t dst);
...
extern int aml_dmx_hw_set_dump_ts_select(struct dmx_demux *demux,
					 int dump_ts_select);
...
extern int dmx_init_sub_buffer(struct aml_dmx *dmx, unsigned long base,
			       unsigned long virt);
