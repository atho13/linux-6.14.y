...
static void ascot3_release(struct dvb_frontend *fe)
{
	struct ascot3_priv *priv = fe->tuner_priv;
...
static enum ascot3_tv_system_t ascot3_get_tv_system(struct dvb_frontend *fe)
{
	...
	dev_dbg(&priv->i2c->dev,
		"%s(): ASCOT3 DTV system %d (delsys %d, bandwidth %d)\n",
...
static const struct dvb_tuner_ops ascot3_tuner_ops = {
	.info = {
		.name = "Sony ASCOT3",
...
EXPORT_SYMBOL_GPL(ascot3_attach);

MODULE_DESCRIPTION("Sony ASCOT3 Terrestrial/Cable Tuner Driver");
