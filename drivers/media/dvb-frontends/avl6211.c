...
/*
 * Availink AVL6211+AV2011 DVB-S/S2 Demod + Tuner Driver
 *
...
const struct avl6211_pllconf pll_conf[] = {
	...
	/* Performance     Ref    Demod   FEC     MPEG  */
	{ 566, 1, 7, 4, 2, 4000,  12600,  18900,  28350 },
	{ 503, 1, 7, 4, 2, 4500,  12600,  18900,  28350 },
	{ 566, 4, 7, 4, 2, 10000, 12600,  18900,  28350 },
	{ 566, 7, 7, 4, 2, 16000, 12600,  18900,  28350 },
	{ 377, 8, 7, 4, 2, 27000, 12600,  18900,  28350 }
};

const unsigned short pll_array_size =
	sizeof(pll_conf) / sizeof(struct avl6211_pllconf);
...
static int avl6211_i2c_writereg(struct avl6211_state *state, u8 *data,
				u16 *size)
{
	int ret;
	...
	if (ret == 1) {
		...
		dev_warn(&state->i2c->dev, "i2c write failed=%d", ret);
		ret = -EREMOTEIO;
	}
...
static int avl6211_i2c_readreg(struct avl6211_state *state, u8 *data, u16 *size)
{
	...
	if (ret == 1) {
		...
		dev_warn(&state->i2c->dev, "i2c read failed=%d", ret);
		ret = -EREMOTEIO;
	}
...
static int avl6211_i2c_read(struct avl6211_state *state, u32 offset, u8 *buf,
			    u16 buf_size)
{
	int ret;
	u8 buf_tmp[3];
...
static int avl6211_i2c_repeater_get_status(struct avl6211_state *state)
{
	...
	ret = avl6211_i2c_read(state, i2cm_cmd_addr + I2CM_CMD_LENGTH - 2, buf,
			       2);
	if (ret)
		goto err;
...
static int avl6211_i2c_repeater_exec(struct avl6211_state *state, u8 *buf,
				     u8 size)
{
	int ret, i = 0;
...
static int avl6211_i2c_repeater_send(struct avl6211_state *state, u8 *buf,
				     u16 size)
{
	int ret;
	...
	/* skip one byte if the size +3 is odd */
	i = 3 + ((3 + size) % 2);
...
static int avl6211_i2c_repeater_recv(struct avl6211_state *state, u8 *buf,
				     u16 size)
{
	int ret, i = 0;
...
static int AV2011_I2C_write(u8 reg_start, u8 *buff, u8 len,
			    struct avl6211_state *state)
{
	int ret, i = 0;
	u8 ucTemp[50] = { 0 };
...
static int avl6211_diseqc_init(struct dvb_frontend *fe)
{
	...
	if (ret)
		goto err;
	/* 2M = 200 * 10kHz */
	ret = avl6211_i2c_write32(state, diseqc_samp_frac_n_addr, 200);
	if (ret)
		goto err;
	ret = avl6211_i2c_write32(state, diseqc_samp_frac_d_addr,
				  state->demod_freq);
	if (ret)
		goto err;
	...
	if (ret)
		goto err;
	ret = avl6211_i2c_write32(state, diseqc_tone_frac_d_addr,
				  state->demod_freq * 10);
	if (ret)
		goto err;
...
static int
avl6211_diseqc_get_tx_status(struct dvb_frontend *fe,
			     struct avl6211_diseqc_tx_status *pTxStatus)
{
	struct avl6211_state *state = fe->demodulator_priv;
	int ret;
	u32 x1;
...
static int avl6211_diseqc_send_mod_data(struct dvb_frontend *fe, const u8 *buf,
					u8 size)
{
	struct avl6211_state *state = fe->demodulator_priv;
	int ret;
	...
	if (WARN_ON(size > 8))
		...
		if (state->diseqc_status == DISEQC_STATUS_CONTINUOUS) {
			ret = avl6211_i2c_read32(state, diseqc_tx_cntrl_addr,
						 &x1);
			if (ret)
				goto err;
			if ((x1 >> 10) & 0x01) {
		...
		if (ret)
			goto err;
		ret = avl6211_i2c_write32(state, diseqc_rx_cntrl_addr,
					  (x2 | 0x01));
		if (ret)
			goto err;
		ret = avl6211_i2c_write32(state, diseqc_rx_cntrl_addr,
		...
		if (Continuousflag == 1)
		{
			...
			x1 |= 0x03;
			ret = avl6211_i2c_write32(state, diseqc_tx_cntrl_addr,
						  x1);
			if (ret)
				goto err;
...
static int avl6211_send_diseqc_msg(struct dvb_frontend *fe,
...
static int avl6211_diseqc_send_burst(struct dvb_frontend *fe,
			       ...
			       enum fe_sec_voltage voltage)
{
	...
	if (voltage == SEC_VOLTAGE_OFF) {
		if (state->config->set_external_vol_gpio)
			state->config->set_external_vol_gpio(&state->demod_id,
							     0);
	...
	*snr = 0;
	...
	if (state->locked == 1) {
		...
		if (ret)
			goto err;
		if (r_snr < 10000) {
			ret = avl6211_i2c_read32(state, rs_code_rate_addr,
						 &code_rate);
			if (ret)
				goto err;
			ret = avl6211_i2c_read32(state, rs_modulation_addr,
						 &modulation);
			if (ret)
				goto err;
...
struct Signal_Level AGC_LUT[91] = {
	{ 63688, 0 },	{ 62626, -1 },	{ 61840, -2 },	{ 61175, -3 },
	{ 60626, -4 },	{ 60120, -5 },	{ 59647, -6 },	{ 59187, -7 },
	{ 58741, -8 },	{ 58293, -9 },	{ 57822, -10 }, { 57387, -11 },
	{ 56913, -12 }, { 56491, -13 }, { 55755, -14 }, { 55266, -15 },
	{ 54765, -16 }, { 54221, -17 }, { 53710, -18 }, { 53244, -19 },
	{ 52625, -20 }, { 52043, -21 }, { 51468, -22 }, { 50904, -23 },
	{ 50331, -24 }, { 49772, -25 }, { 49260, -26 }, { 48730, -27 },
	{ 48285, -28 }, { 47804, -29 }, { 47333, -30 }, { 46880, -31 },
	{ 46460, -32 }, { 46000, -33 }, { 45539, -34 }, { 45066, -35 },
	{ 44621, -36 }, { 44107, -37 }, { 43611, -38 }, { 43082, -39 },
	{ 42512, -40 }, { 41947, -41 }, { 41284, -42 }, { 40531, -43 },
	{ 39813, -44 }, { 38978, -45 }, { 38153, -46 }, { 37294, -47 },
	{ 36498, -48 }, { 35714, -49 }, { 35010, -50 }, { 34432, -51 },
	{ 33814, -52 }, { 33315, -53 }, { 32989, -54 }, { 32504, -55 },
	{ 32039, -56 }, { 31608, -57 }, { 31141, -58 }, { 30675, -59 },
	{ 30215, -60 }, { 29711, -61 }, { 29218, -62 }, { 28688, -63 },
	{ 28183, -64 }, { 27593, -65 }, { 26978, -66 }, { 26344, -67 },
	{ 25680, -68 }, { 24988, -69 }, { 24121, -70 }, { 23285, -71 },
	{ 22460, -72 }, { 21496, -73 }, { 20495, -74 }, { 19320, -75 },
	{ 18132, -76 }, { 16926, -77 }, { 15564, -78 }, { 14398, -79 },
	{ 12875, -80 }, { 11913, -81 }, { 10514, -82 }, { 9070, -83 },
	{ 7588, -84 },	{ 6044, -85 },	{ 4613, -86 },	{ 3177, -87 },
	{ 1614, -88 },	{ 123, -89 },	{ 0, -90 }
};

static int avl6211_read_signal_strength(struct dvb_frontend *fe,
					u16 *signal_strength)
{
	...
	if (state->locked == 1) {
		...
		if (i <= Level_High_Stage)
			Percent = Percent_Space_Low + Percent_Space_Mid +
				  (Level_High_Stage - i) * Percent_Space_High /
					  Level_High_Stage;
		else if (i <= Level_Low_Stage)
			Percent = Percent_Space_Low +
				  (Level_Low_Stage - i) * Percent_Space_Mid /
					  (Level_Low_Stage - Level_High_Stage);
		else
			Percent = (90 - i) * Percent_Space_Low /
				  (90 - Level_Low_Stage);
...
static int avl6211_channel_lock(struct dvb_frontend *fe)
{
	...
	Standard = (u16)(((state->flags) & CI_FLAG_DVBS2_BIT_MASK) >>
			 CI_FLAG_DVBS2_BIT);
	autoIQ_Detect = (((state->flags) & CI_FLAG_IQ_AUTO_BIT_MASK) >>
			 CI_FLAG_IQ_AUTO_BIT);
	auto_manual_lock =
		(u16)(((state->flags) & CI_FLAG_MANUAL_LOCK_MODE_BIT_MASK) >>
	...
	ret = avl6211_i2c_write16(state, rc_fec_bypass_coderate_addr,
				  auto_manual_lock);
	if (ret)
		goto err;
	...
	if (ret)
		goto err;
	ret = avl6211_i2c_write32(state, rc_int_sym_rate_MHz_addr,
				  state->symbol_rate);
	if (ret)
		goto err;
...
static int avl6211_set_frontend(struct dvb_frontend *fe)
{
	...
	/* Max wait for tuner locking in ms */
	max_time = 150;
	...
	do {
		...
		if (!ret)
			...
			/* Wait for demod to lock the channel */
			msleep(10);
			continue;
		}
	...
	state->flags |=
		...
		<< CI_FLAG_IQ_AUTO_BIT; //Enable automatic IQ swap detection
	state->flags |=
		(CI_FLAG_DVBS2_UNDEF)
		<< CI_FLAG_DVBS2_BIT; //Enable automatic standard detection
	...
	cnt = max_time / 10;
	do {
		ret = avl6211_i2c_read16(state, rs_fec_lock_addr,
					 &state->locked);
...
static int avl6211_setup_pll(struct avl6211_state *state,
			     const struct avl6211_pllconf *pll_ptr)
{
	int ret;
...
static int avl6211_load_firmware(struct dvb_frontend *fe)
{
	...
	if (ret) {
		dev_dbg(&state->i2c->dev,
			"Firmware upload failed. Timeout or file not found\n");
		goto err;
	}
	...
	if (!buffer) {
		...
		fw = NULL;
		dev_dbg(&state->i2c->dev,
			"Failed to allocate tmp memory for firmware\n");
		return -ENOMEM;
	}
	...
	while (i < data_size) {
		...
		i += 4;
		ret = avl6211_i2c_write(state, buffer + i + 1,
					(u16)(buf_size + 3));
		if (ret)
			goto err;
...
static int avl6211_init(struct dvb_frontend *fe)
{
	...
	ret = avl6211_i2c_write16(state, rc_int_dmd_clk_MHz_addr,
				  state->demod_freq);
	if (ret)
		goto err;
	ret = avl6211_i2c_write16(state, rc_int_fec_clk_MHz_addr,
				  state->fec_freq);
	if (ret)
		goto err;
	ret = avl6211_i2c_write16(state, rc_int_mpeg_clk_MHz_addr,
				  state->mpeg_freq);
	if (ret)
		goto err;
	...
	ret = avl6211_i2c_write32(state, rc_rfagc_pol_addr,
				  (u32)state->config->tuner_rfagc);
	if (ret)
		goto err;
	...
	ret = avl6211_i2c_write16(state,
				  rc_blind_scan_tuner_spectrum_inversion_addr,
				  (u16)state->config->tuner_spectrum);
	if (ret)
		goto err;

	ret = avl6211_i2c_write32(state, rc_mpeg_mode_addr,
				  (u32)(state->config->mpeg_format));
	if (ret)
		goto err;
	ret = avl6211_i2c_write16(state, rc_mpeg_serial_addr,
				  (u16)(state->config->mpeg_mode));
	if (ret)
		goto err;
	ret = avl6211_i2c_write16(state, rc_mpeg_posedge_addr,
				  (u16)(state->config->mpeg_pol));
	if (ret)
		goto err;

	if (state->config->mpeg_mode) {
		ret = avl6211_i2c_write32(state, rc_outpin_sel_addr,
					  (u32)(state->config->mpeg_pin));
		if (ret)
			goto err;
	}
...
static struct dvb_frontend_ops avl6211_ops = {
	...
	.info = { .name = "Availink AVL6211+AV2011 DVB-S/S2",
		  ...
		  .caps = FE_CAN_INVERSION_AUTO | FE_CAN_FEC_1_2 |
			  ...
			  FE_CAN_FEC_8_9 | FE_CAN_FEC_AUTO | FE_CAN_QPSK |
			  FE_CAN_RECOVER | FE_CAN_2G_MODULATION },
...
struct dvb_frontend *avl6211_attach(struct i2c_adapter *i2c,
				    struct avl6211_config *config, int id)

{
	...
	memcpy(&state->frontend.ops, &avl6211_ops,
