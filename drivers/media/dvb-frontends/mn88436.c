...
static int mn88436_write_reg(struct mn88436_state *state, u8 id, u8 reg, u8 val)
{
	...
	if (ret == 1) {
		...
		dev_warn(&state->i2c->dev, "i2c wr failed=%d reg=%02x", ret,
			 reg);
		ret = -EREMOTEIO;
	}
	return ret;
...
static int mn88436_read_reg(struct mn88436_state *state, u8 id, u8 reg, u8 *val)
{
	...
	if (ret == 2) {
		...
		dev_warn(&state->i2c->dev, "i2c rd failed=%d reg=%02x", ret,
			 reg);
		ret = -EREMOTEIO;
	}
	return ret;
...
static int mn88436_write_reg_mask(struct mn88436_state *state, u8 id, u8 reg,
				  u8 mask, u8 data)
{
	int ret;
	u8 rd;
...
static int mn88436_init(struct dvb_frontend *fe)
{
	...
	for (i = 0;;) {
		...
		ret = mn88436_write_reg(state, fw->data[i], fw->data[i + 1],
					fw->data[i + 2]);
		if (ret)
			goto err2;
	...
	if (ret) {
		dev_dbg(&state->i2c->dev,
			"Pseq upload failed. Timeout or file not found\n");
		goto err1;
	}
	...
	for (i = 0; i < fw->size; i++) {
		ret = mn88436_write_reg(state, 0, DMD_MAIN_PSEQPRG,
					fw->data[i]);
		if (ret)
			goto err2;
	}
...
struct dvb_frontend *mn88436_attach(struct i2c_adapter *i2c, u8 device_id)
{
...
EXPORT_SYMBOL_GPL(mn88436_attach);
