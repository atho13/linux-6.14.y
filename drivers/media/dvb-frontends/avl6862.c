...
static int avl6862_i2c_wrm(struct avl6862_priv *priv, u8 *buf, int len)
{
	...
	struct i2c_msg msg = {
		.addr = priv->config->demod_address,
		.flags =
			0, //I2C_M_NOSTART,	/*i2c_transfer will emit a stop flag, so we should send 2 msg together,
...
static int avl6862_i2c_wr_data(struct avl6862_priv *priv, u32 addr, u32 *data,
			       int len)
{
	int ret = 0, this_len;
...
static int avl6862_i2c_wr_reg(struct avl6862_priv *priv, u32 addr, u32 data,
			      int reg_size)
{
	u8 buf[3 + 4];
	u8 *p = buf;
...
static int avl6862_i2c_rd_reg(struct avl6862_priv *priv, u32 addr, u32 *data,
			      int reg_size)
{
	int ret;
	u8 buf[3 + 4];
...
static int avl6862_wait_demod(struct avl6862_priv *priv)
{
	...
	do {
		...
		ret = avl6862_RD_REG16(priv, 0x200 + rc_fw_command_saddr_offset,
				       &cmd);
		if ((ret == 0) && (cmd == 0))
			return ret;
		//		else
...
static int avl6862_exec_n_wait(struct avl6862_priv *priv, u8 cmd)
{
	...
	if (ret)
		return ret;
*/
	ret = avl6862_WR_REG16(priv, 0x200 + rc_fw_command_saddr_offset,
			       (u32)cmd);
	if (ret)
		return ret;
	...
	while (idx < len) {
		...
		if (num_cond_words == 0) {
			...
			for (i = 0; i < num_cond_words; i++) {
				...
				if ((addr_mode_op ==
				     PATCH_OP_ADDR_MODE_VAR_IDX) &&
				    (binary_op != PATCH_OP_BINARY_STORE)) {
					value = variable_array
						[value]; //grab variable value
				}
		...
		if (condition) {
			...
			case PATCH_CMD_LD_TO_DEVICE:
				...
				ret = avl6862_i2c_wr_data(priv, dest_addr,
							  &patch[data_offset],
							  length);
				num_rvs = patch[idx++];
				break;
			case PATCH_CMD_LD_TO_DEVICE_IMM:
				...
				dest_addr = patch[idx++];
				data = patch[idx++];
				ret = avl6862_i2c_wr_reg(priv, dest_addr, data,
							 length);
				num_rvs = patch[idx++];
				break;
			case PATCH_CMD_RD_FROM_DEVICE:
				...
				src_addr = patch[idx++];
				num_rvs = patch[idx++];
				ret = avl6862_i2c_rd_reg(priv, src_addr, &data,
							 length);
				i = patch[idx];
			...
			case PATCH_CMD_DECOMPRESS:
				...
				avl6862_WR_REG32(priv, args_addr + 8,
						 dest_addr);
				if (type == PATCH_CMP_TYPE_ZLIB) {
					avl6862_WR_REG32(priv, args_addr + 12,
							 ref_addr);
					avl6862_WR_REG32(priv, args_addr + 16,
							 ref_size);
				}
				ret = avl6862_exec_n_wait(
					priv, AVL_FW_CMD_DECOMPRESS);
				num_rvs = patch[idx++];
				break;
			...
			case PATCH_CMD_DMA_HW:
				...
				do {
					if (i++ > DMA_MAX_TRIES)
						return -ENODEV;
					ret |= avl6862_RD_REG32(priv, 0x110048,
								&data);
				} while (!(0x01 & data));

				if (data)
					ret |= avl6862_WR_REG32(priv, 0x110050,
								dest_addr);
				idx += length * 3;
				num_rvs = patch[idx++];
				break;
...
static int avl6862_wait_demod_boot(struct avl6862_priv *priv)
{
	...
	do {
		ret = avl6862_RD_REG32(priv, 0x110840,
				       &status); // hw_mcu_reset_base
		ret |= avl6862_RD_REG32(priv, rs_core_ready_word_iaddr_offset,
					&ready_code);
...
static int avl6862_load_firmware(struct avl6862_priv *priv)
{
	...
	case SYS_DVBC_ANNEX_B:
		...
		dev_info(&priv->i2c->dev,
			 "Load avl6862 firmware patch for DVB-C size=%d",
			 fw_size);
		break;
	case SYS_DVBS:
	case SYS_DVBS2:
		...
		dev_info(&priv->i2c->dev,
			 "Load avl6862 firmware patch for DVB-S/S2 size=%d",
			 fw_size);
		break;
	case SYS_DVBT:
	case SYS_DVBT2:
		...
		dev_info(&priv->i2c->dev,
			 "Load avl6862 firmware patch for DVB-T/T2 size=%d",
			 fw_size);
		break;
	default:
		ret = -EINVAL;
	...
	for (i = 0; i < fw_size; i += 4) {
		*(ptr++) = (fw_data[i] << 24) | (fw_data[i + 1] << 16) |
			   (fw_data[i + 2] << 8) | fw_data[i + 3];
	}
	...
	ret |= avl6862_WR_REG32(priv, 0x0a4 + rs_core_ready_word_iaddr_offset,
				0x00000000);
	...
	dev_info(&priv->i2c->dev, "avl6862 patch ver %d.%d build %d",
		 (i >> 24) & 0xFF, (i >> 16) & 0xFF, i & 0xFFFF);
			...
			AVL_ErrorStatConfig stErrorStatConfig)
{
	...
	r = avl6862_WR_REG32(priv, 0x132050 + esm_mode_offset,
			     (u32)stErrorStatConfig.eErrorStatMode);
	r |= avl6862_WR_REG32(priv, 0x132050 + tick_type_offset,
			      (u32)stErrorStatConfig.eAutoErrorStatType);

	r |= avl6862_WR_REG32(priv, 0x132050 + time_tick_low_offset,
			      (u32)(time_tick_num));
	r |= avl6862_WR_REG32(priv, 0x132050 + time_tick_high_offset,
			      (u32)(time_tick_num >> 32));

	r |= avl6862_WR_REG32(priv, 0x132050 + byte_tick_low_offset,
			      stErrorStatConfig.uiTimeThresholdMs);
	r |= avl6862_WR_REG32(priv, 0x132050 + byte_tick_high_offset,
				  ...
				  AVL_Diseqc_Para *pDiseqcPara)
{
	...
	r |= avl6862_WR_REG32(priv, 0x16c000 + hw_diseqc_samp_frac_n_offset,
			      2000000); //2M=200*10kHz
	r |= avl6862_WR_REG32(priv, 0x16c000 + hw_diseqc_samp_frac_d_offset,
			      166666667); //uiDDCFrequencyHz  166666667

	r |= avl6862_WR_REG32(priv, 0x16c000 + hw_diseqc_tone_frac_n_offset,
			      ((pDiseqcPara->uiToneFrequencyKHz) << 1));
	r |= avl6862_WR_REG32(priv, 0x16c000 + hw_diseqc_tone_frac_d_offset,
			      (166666667 / 1000)); //uiDDCFrequencyHz  166666667
...
static int avl6862_init_dvbs(struct dvb_frontend *fe)
{
	...
	ret = avl6862_WR_REG16(
		priv, 0xe00 + rc_DVBSx_int_mpeg_clk_MHz_saddr_offset, 27000);
	ret |= avl6862_WR_REG16(
		priv, 0xe00 + rc_DVBSx_int_fec_clk_MHz_saddr_offset, 25000);

	ret |= avl6862_WR_REG16(priv,
				0xe00 + rc_DVBSx_int_adc_clk_MHz_saddr_offset,
				12500); // uiADCFrequencyHz  125000000
	ret |= avl6862_WR_REG16(
		priv, 0xe00 + rc_DVBSx_int_dmd_clk_MHz_saddr_offset,
		166666667 / 10000); //uiDDCFrequencyHz  166666667

	ret |= avl6862_WR_REG32(priv, 0xe00 + rc_DVBSx_rfagc_pol_iaddr_offset,
				AVL_AGC_INVERTED);

	ret |= avl6862_WR_REG32(priv, 0xe00 + rc_DVBSx_format_iaddr_offset,
				AVL_OFFBIN); //Offbin
	ret |= avl6862_WR_REG32(priv, 0xe00 + rc_DVBSx_input_iaddr_offset,
				AVL_ADC_IN); //ADC in

	ret |= avl6862_WR_REG16(
		priv, 0xe00 + rc_DVBSx_IF_Offset_10kHz_saddr_offset, 0);
...
static int avl6862_init_dvbc(struct dvb_frontend *fe)
{
	...
	ret = avl6862_WR_REG32(priv, 0x600 + rc_DVBC_dmd_clk_Hz_iaddr_offset,
			       250000000);
	ret |= avl6862_WR_REG32(priv, 0x600 + rc_DVBC_fec_clk_Hz_iaddr_offset,
				250000000);
	ret |= avl6862_WR_REG8(priv, 0x600 + rc_DVBC_rfagc_pol_caddr_offset,
			       AVL_AGC_NORMAL);
	ret |= avl6862_WR_REG32(priv, 0x600 + rc_DVBC_if_freq_Hz_iaddr_offset,
				5000000);
	ret |= avl6862_WR_REG8(priv, 0x600 + rc_DVBC_adc_sel_caddr_offset,
			       (u8)AVL_IF_Q);
	ret |= avl6862_WR_REG32(
		priv, 0x600 + rc_DVBC_symbol_rate_Hz_iaddr_offset, 6875000);
	ret |= avl6862_WR_REG8(priv, 0x600 + rc_DVBC_j83b_mode_caddr_offset,
			       AVL_DVBC_J83A);

	//DDC configuration
	ret |= avl6862_WR_REG8(priv, 0x600 + rc_DVBC_input_format_caddr_offset,
			       AVL_ADC_IN); //ADC in
	ret |= avl6862_WR_REG8(priv, 0x600 + rc_DVBC_input_select_caddr_offset,
			       AVL_OFFBIN); //RX_OFFBIN
	ret |= avl6862_WR_REG8(priv, 0x600 + rc_DVBC_tuner_type_caddr_offset,
			       AVL_DVBC_IF); //IF

	//ADC configuration
	ret |= avl6862_WR_REG8(priv,
			       0x600 + rc_DVBC_adc_use_pll_clk_caddr_offset, 0);
	ret |= avl6862_WR_REG32(
		priv, 0x600 + rc_DVBC_sample_rate_Hz_iaddr_offset, 30000000);
...
static int avl6862_init_dvbc_b(struct dvb_frontend *fe)
{
	...
	ret |= avl6862_WR_REG8(priv, 0x600 + rc_DVBC_adc_sel_caddr_offset,
			       (u8)AVL_IF_Q);
	ret |= avl6862_WR_REG32(
		priv, 0x600 + rc_DVBC_symbol_rate_Hz_iaddr_offset, 5360000);
	ret |= avl6862_WR_REG8(priv, 0x600 + rc_DVBC_j83b_mode_caddr_offset,
			       AVL_DVBC_J83B);
...
static int avl6862_init_dvbt(struct dvb_frontend *fe)
{
	...
	ret = avl6862_WR_REG32(
		priv, 0xa00 + rc_DVBTx_sample_rate_Hz_iaddr_offset, 30000000);
	ret |= avl6862_WR_REG32(priv,
				0xa00 + rc_DVBTx_mpeg_clk_rate_Hz_iaddr_offset,
				270000000);

	/* DDC configuration */
	ret |= avl6862_WR_REG8(priv, 0xa00 + rc_DVBTx_input_format_caddr_offset,
			       AVL_OFFBIN);
	ret |= avl6862_WR_REG8(priv, 0xa00 + rc_DVBTx_input_select_caddr_offset,
			       AVL_ADC_IN);
	ret |= avl6862_WR_REG8(priv, 0xa00 + rc_DVBTx_tuner_type_caddr_offset,
			       AVL_DVBTX_REAL_IF);
	ret |= avl6862_WR_REG8(priv, 0xa00 + rc_DVBTx_rf_agc_pol_caddr_offset,
			       0);
	ret |= avl6862_WR_REG32(
		priv, 0xa00 + rc_DVBTx_nom_carrier_freq_Hz_iaddr_offset,
		5000000);

	/* ADC configuration */
	ret |= avl6862_WR_REG8(priv, 0xa00 + rc_DVBTx_adc_sel_caddr_offset,
			       (u8)AVL_IF_Q);
	ret |= avl6862_WR_REG8(
		priv, 0xa00 + rc_DVBTx_adc_use_pll_clk_caddr_offset, 0);
...
static int avl6862_i2c_gate_ctrl(struct dvb_frontend *fe, int enable)
{
	...
	if (enable) {
		ret = avl6862_WR_REG32(
			priv, 0x118000 + tuner_i2c_bit_rpt_cntrl_offset, 0x07);
		ret = avl6862_WR_REG32(
			priv, 0x118000 + tuner_i2c_bit_rpt_cntrl_offset, 0x07);
	} else
		ret = avl6862_WR_REG32(
			priv, 0x118000 + tuner_i2c_bit_rpt_cntrl_offset, 0x06);
...
static int avl6862_set_dvbs(struct dvb_frontend *fe)
{
	...
	ret |= avl6862_WR_REG16(priv, 0xe00 + rc_DVBSx_decode_mode_saddr_offset,
				0x14);
	ret |= avl6862_WR_REG16(
		priv, 0xe00 + rc_DVBSx_fec_bypass_coderate_saddr_offset,
		0); //DVBS auto lock
	ret |= avl6862_WR_REG16(priv, 0xe00 + rc_DVBSx_iq_mode_saddr_offset,
				1); //enable spectrum auto detection
	ret |= avl6862_WR_REG16(priv, 0xe00 + rc_DVBSx_decode_mode_saddr_offset,
				0x14);
	ret |= avl6862_WR_REG16(
		priv, 0xe00 + rc_DVBSx_fec_bypass_coderate_saddr_offset, 0);
	ret |= avl6862_WR_REG32(priv,
				0xe00 + rc_DVBSx_int_sym_rate_MHz_iaddr_offset,
				c->symbol_rate);
...
static int avl6862_set_dvbc(struct dvb_frontend *fe)
{
	...
	ret = avl6862_WR_REG32(
		priv, 0x600 + rc_DVBC_qam_mode_scan_control_iaddr_offset,
		0x0101);
	ret |= avl6862_WR_REG32(priv,
				0x600 + rc_DVBC_symbol_rate_Hz_iaddr_offset,
				c->symbol_rate);
	ret |= avl6862_WR_REG8(priv, 0x600 + rc_DVBC_j83b_mode_caddr_offset,
			       AVL_DVBC_J83A);
	ret |= avl6862_exec_n_wait(priv, AVL_FW_CMD_ACQUIRE);
...
static int avl6862_set_dvbt(struct dvb_frontend *fe)
{
	...
	ret = avl6862_WR_REG32(priv, 0xa00 + rc_DVBTx_fund_rate_Hz_iaddr_offset,
			       bw_fft);
	ret |= avl6862_WR_REG8(priv, 0xa00 + rc_DVBTx_l1_proc_only_caddr_offset,
			       0);

	/* spectrum inversion */
	ret |= avl6862_WR_REG8(priv,
			       0xa00 + rc_DVBTx_spectrum_invert_caddr_offset,
			       AVL_SPECTRUM_AUTO);

	switch (c->delivery_system) {
	case SYS_DVBT:
		ret |= avl6862_WR_REG8(
			priv, 0xa00 + rc_DVBTx_acquire_mode_caddr_offset,
			(u8)AVL_DVBTx_LockMode_T_ONLY);
		ret |= avl6862_WR_REG8(
			priv, 0xa00 + rc_DVBTx_dvbt_layer_select_caddr_offset,
			0);
		ret |= avl6862_WR_REG8(
			priv, 0xa00 + rc_DVBTx_data_PLP_ID_caddr_offset, 0);
		ret |= avl6862_WR_REG8(
			priv, 0xa00 + rc_DVBTx_common_PLP_ID_caddr_offset, 0);
		ret |= avl6862_WR_REG8(
			priv, 0xa00 + rc_DVBTx_common_PLP_present_caddr_offset,
			0);
		break;
	case SYS_DVBT2:
	default:
		ret |= avl6862_WR_REG8(
			priv, 0xa00 + rc_DVBTx_acquire_mode_caddr_offset,
			AVL_DVBTx_LockMode_ALL);
		ret |= avl6862_WR_REG8(
			priv, 0xa00 + rc_DVBTx_data_PLP_ID_caddr_offset,
			c->stream_id);
		ret |= avl6862_WR_REG8(
			priv, 0xa00 + rc_DVBTx_common_PLP_ID_caddr_offset, 0);
		ret |= avl6862_WR_REG8(
...
#define I2C_RPT_DIV \
	((0x2A) * (250000) / (240 * 1000)) //m_CoreFrequency_Hz 250000000
			       ...
			       enum fe_delivery_system delsys)
{
	...
	if (priv->config->ts_serial) {
		ret |= avl6862_WR_REG8(priv, 0x200 + rc_ts_serial_caddr_offset,
				       AVL_TS_SERIAL);
	...
	ret |= avl6862_WR_REG8(priv, 0x200 + rc_ts_clock_edge_caddr_offset,
			       AVL_MPCM_RISING);
	ret |= avl6862_WR_REG8(priv,
			       0x200 + rc_enable_ts_continuous_caddr_offset,
			       AVL_TS_CONTINUOUS_ENABLE);
	ret |= avl6862_WR_REG32(priv,
				0x200 + rc_ts_cntns_clk_frac_d_iaddr_offset,
				uiTSFrequencyHz);
	if (priv->config->ts_serial) {
		switch (priv->delivery_system) {
		case SYS_DVBS:
		case SYS_DVBS2:
			ret |= avl6862_WR_REG32(
				priv,
				0x200 + rc_ts_cntns_clk_frac_n_iaddr_offset,
				uiTSFrequencyHz);
			break;
		case SYS_DVBT:
		case SYS_DVBT2:
		...
		ret |= avl6862_WR_REG32(
			priv, 0x200 + rc_ts_cntns_clk_frac_n_iaddr_offset,
			uiTSFrequencyHz / 8);

	/* TS serial pin */
	ret |= avl6862_WR_REG8(priv, 0x200 + rc_ts_serial_outpin_caddr_offset,
			       AVL_MPSP_DATA0);

	/* TS serial order */
	ret |= avl6862_WR_REG8(priv, 0x200 + rc_ts_serial_msb_caddr_offset,
			       AVL_MPBO_MSB);

	/* TS serial sync pulse */
	ret |= avl6862_WR_REG8(priv, 0x200 + rc_ts_sync_pulse_caddr_offset,
			       AVL_TS_SERIAL_SYNC_1_PULSE);
	/* TS error pol */
	ret |= avl6862_WR_REG8(priv, 0x200 + rc_ts_error_polarity_caddr_offset,
			       AVL_MPEP_Normal);
	/* TS valid pol */
	ret |= avl6862_WR_REG8(priv, 0x200 + rc_ts_valid_polarity_caddr_offset,
			       AVL_MPVP_Normal);
	/* TS packet len */
	ret |= avl6862_WR_REG8(priv, 0x200 + rc_ts_packet_len_caddr_offset,
			       AVL_TS_188);
	/* TS parallel order */
	ret |= avl6862_WR_REG8(priv, 0x200 + rc_ts_packet_order_caddr_offset,
			       AVL_TS_PARALLEL_ORDER_NORMAL);
	/* TS parallel phase */
	ret |= avl6862_WR_REG8(priv, 0x200 + ts_clock_phase_caddr_offset,
			       AVL_TS_PARALLEL_PHASE_0);
	...
	/* close gate */
	ret |= avl6862_WR_REG32(priv, 0x118000 + tuner_i2c_bit_rpt_cntrl_offset,
				0x6);
	...
	/* set bit clock */
	ret |= avl6862_WR_REG32(
		priv, 0x118000 + tuner_i2c_bit_rpt_clk_div_offset, I2C_RPT_DIV);
	/* release from reset */
...
static int AVL_Demod_DVBSx_Diseqc_SendModulationData(struct avl6862_priv *priv,
						     AVL_puchar pucBuff,
						     u8 ucSize)
{
	int r = 0;
	...
	if (ucSize > 8) {
		...
		if (priv->config->eDiseqcStatus == AVL_DOS_InContinuous) {
			r |= avl6862_RD_REG32(
				priv, 0x16c000 + hw_diseqc_tx_cntrl_offset,
				&i1);
			if ((i1 >> 10) & 0x01) {
				Continuousflag = 1;
				i1 &= 0xfffff3ff;
				r |= avl6862_WR_REG32(
					priv,
					0x16c000 + hw_diseqc_tx_cntrl_offset,
					i1);
		...
		//reset rx_fifo
		r |= avl6862_RD_REG32(
			priv, 0x16c000 + hw_diseqc_rx_cntrl_offset, &i2);
		r |= avl6862_WR_REG32(priv,
				      0x16c000 + hw_diseqc_rx_cntrl_offset,
				      (i2 | 0x01));
		r |= avl6862_WR_REG32(priv,
				      0x16c000 + hw_diseqc_rx_cntrl_offset,
				      (i2 & 0xfffffffe));
		...
		r |= avl6862_RD_REG32(
			priv, 0x16c000 + hw_diseqc_tx_cntrl_offset, &i1);
		i1 &= 0xfffffff8; //set to modulation mode and put it to FIFO load mode
		r |= avl6862_WR_REG32(priv,
				      0x16c000 + hw_diseqc_tx_cntrl_offset, i1);

		for (i2 = 0; i2 < ucSize; i2++) {
			r |= avl6862_WR_REG32(priv,
					      0x16c000 + hw_tx_fifo_map_offset,
					      pucBuff[i2]);
		}
		...
		do {
			...
			r = avl6862_RD_REG32(
				priv, 0x16c000 + hw_diseqc_tx_st_offset, &i1);
				       ...
				       AVL_Diseqc_TxStatus *pTxStatus)
{
	...
	if ((AVL_DOS_InModulation == priv->config->eDiseqcStatus) ||
	    (AVL_DOS_InTone == priv->config->eDiseqcStatus)) {
		r |= avl6862_RD_REG32(priv, 0x16c000 + hw_diseqc_tx_st_offset,
				      &i1);
...
int AVL_SX_DiseqcSendCmd(struct avl6862_priv *priv, AVL_puchar pCmd, u8 CmdSize)
{
	...
	if (r != AVL_EC_OK) {
		...
		do {
			msleep(5);
			r |= AVL_Demod_DVBSx_Diseqc_GetTxStatus(priv,
								&TxStatus);
				    ...
				    u8 ucCount)
{
	...
	if (ucCount > 8) {
		...
		for (i2 = 0; i2 < ucCount; i2++) {
			r |= avl6862_WR_REG32(
				priv, 0x16c000 + hw_tx_fifo_map_offset, 1);
		}
...
static int avl6862_burst(struct dvb_frontend *fe, enum fe_sec_mini_cmd burst)
{
	...
	ret = AVL_Demod_DVBSx_Diseqc_SendTone(priv, burst == SEC_MINI_A ? 1 : 0,
					      1);
...
static int avl6862_set_tone(struct dvb_frontend *fe, enum fe_sec_tone_mode tone)
{
	...
	dbg_avl("tone: %d", tone);
	ret = avl6862_RD_REG32(priv, 0x16c000 + hw_diseqc_tx_cntrl_offset,
			       &reg);
	if (ret)
		return ret;
	...
	return avl6862_WR_REG32(priv, 0x16c000 + hw_diseqc_tx_cntrl_offset,
				reg);
}

static int avl6862_set_voltage(struct dvb_frontend *fe,
...
static int avl6862_read_status(struct dvb_frontend *fe, enum fe_status *status)
{
	...
	case SYS_DVBC_ANNEX_A:
	case SYS_DVBC_ANNEX_B:
		ret |= avl6862_RD_REG32(
			priv, 0x400 + rs_DVBC_mode_status_iaddr_offset, &reg);
		if ((reg & 0xff) == 0x15)
			reg = 1;
		else
			reg = 0;
		if (reg) {
			ret |= avl6862_RD_REG16(
				priv, 0x400 + rs_DVBC_snr_dB_x100_saddr_offset,
				&snr);
			if (ret)
				snr = 0;
		}
	...
	case SYS_DVBS2:
		ret |= avl6862_RD_REG16(
			priv, 0xc00 + rs_DVBSx_fec_lock_saddr_offset, &reg);
		if (reg) {
			ret |= avl6862_RD_REG32(
				priv, 0xc00 + rs_DVBSx_int_SNR_dB_iaddr_offset,
				&snr);
			if (ret || snr > 10000)
				snr = 0;
		} else {
	...
	case SYS_DVBT2:
		ret |= avl6862_RD_REG8(
			priv, 0x800 + rs_DVBTx_fec_lock_caddr_offset, &reg);
		if (reg) {
			ret |= avl6862_RD_REG16(
				priv, 0x800 + rs_DVBTx_snr_dB_x100_saddr_offset,
				&snr);
			if (ret)
				snr = 0;
		}
	...
	*status = FE_HAS_SIGNAL;
	...
	if (Level <= Level_High_Stage)
		Percent = Percent_Space_Low + Percent_Space_Mid +
			  (Level_High_Stage - Level) * Percent_Space_High /
				  Level_High_Stage;
	else if (Level <= Level_Low_Stage)
		Percent = Percent_Space_Low +
			  (Level_Low_Stage - Level) * Percent_Space_Mid /
				  (Level_Low_Stage - Level_High_Stage);
	else
		Percent = (90 - Level) * Percent_Space_Low /
			  (90 - Level_Low_Stage);

	c->strength.stat[1].scale = FE_SCALE_RELATIVE;
	c->strength.stat[1].uvalue =
		(Percent * 65535) / 100; //(100 - agc/1000) * 656;
...
static int avl6862_read_ber(struct dvb_frontend *fe, u32 *ber)
{
	...
	*ber = 10e7;
	...
	case SYS_DVBS2:
		ret = avl6862_RD_REG32(
			priv,
			0xc00 + rs_DVBSx_post_LDPC_BER_estimate_x10M_iaddr_offset,
			ber);
		break;
	case SYS_DVBT:
		ret = avl6862_RD_REG32(
	...
	case SYS_DVBT2:
		ret = avl6862_RD_REG32(
			priv,
			0x800 + rs_DVBTx_post_LDPC_BER_estimate_x1B_iaddr_offset,
			ber);
		if (!ret)
			*ber /= 100;
		break;
...
static int avl6862_set_frontend(struct dvb_frontend *fe)
{
	...
	ret = avl6862_RD_REG32(
		priv, 0x200 + rs_current_active_mode_iaddr_offset, &demod_mode);
	if (ret)
		return ret;
	...
	case SYS_DVBT2:
		...
		ret = avl6862_RD_REG32(
			priv, 0x200 + rs_current_active_mode_iaddr_offset,
			&demod_mode);
		if (ret || demod_mode != AVL_DVBTX) {
			dev_err(&priv->i2c->dev,
				"%s: failed to enter DVBTx mode",
				KBUILD_MODNAME);
			ret = -EAGAIN;
			break;
		}
	...
	case SYS_DVBC_ANNEX_B:
		...
		if (ret || demod_mode != AVL_DVBC) {
			dev_err(&priv->i2c->dev,
				"%s: failed to enter DVBC annex B mode",
				KBUILD_MODNAME);
			ret = -EAGAIN;
			break;
		}
...
static struct dvb_frontend_ops avl6862_ops = {
	...
	.info = { .name = "Availink avl6862",
		  ...
		  .caps = FE_CAN_FEC_1_2 | FE_CAN_FEC_2_3 | FE_CAN_FEC_3_4 |
			  ...
			  FE_CAN_FEC_7_8 | FE_CAN_FEC_AUTO | FE_CAN_QPSK |
			  FE_CAN_QAM_16 | FE_CAN_QAM_32 | FE_CAN_QAM_64 |
			  FE_CAN_QAM_128 | FE_CAN_QAM_256 | FE_CAN_QAM_AUTO |
			  FE_CAN_TRANSMISSION_MODE_AUTO |
			  FE_CAN_GUARD_INTERVAL_AUTO | FE_CAN_HIERARCHY_AUTO |
			  FE_CAN_MUTE_TS | FE_CAN_2G_MODULATION |
			  FE_CAN_MULTISTREAM | FE_CAN_INVERSION_AUTO },
				    ...
				    struct i2c_adapter *i2c)
{
	...
	if (config->dual_tuner)
		...
		memcpy(&priv->frontend.ops, &avl6762_ops,
		       sizeof(struct dvb_frontend_ops));
	...
	priv->i2c = i2c;
	priv->g_nChannel_ts_total = 0, priv->delivery_system = -1;
