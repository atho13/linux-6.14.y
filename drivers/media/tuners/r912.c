...
/*
 * Rafael Micro R912 silicon tuner driver
 *
 * Based on Rafael Micro vendor driver
 * Author: Ryan Chang
 *
...
#define dev_dbg(fmt, args...)                                         \
	do {                                                          \
		/* Burada yeni değişken ismini kullanıyoruz */        \
		if (debug_r912)                                       \
			pr_debug("R912: %s: " fmt, __func__, ##args); \
	} while (0)

/* Değişken ismini debug_r912 olarak güncelledik */
static int debug_r912;
module_param_named(debug_r912, debug_r912, int, 0644);
MODULE_PARM_DESC(debug_r912, "Enable Rafael R912 tuner debug information");
...
u8 R912_iniArray_hybrid[R912_REG_NUM] = {
	...
	0x7B, 0x0B, 0x70, 0x0A, 0x6E, 0x20, 0x70, 0x87, 0x96, 0x00,
	//adr:0x19 from 0x83(1.05+-0.25) modify to 0x87 (1.4+-0.6)
...
u8 R912_iniArray_dvbs[R912_REG_NUM] = {
	...
	//  0x08  0x09  0x0A  0x0B  0x0C  0x0D  0x0E  0x0F  0x10  0x11
	0xF0, 0x0F, 0x4D, 0x0A, 0x6F, 0x20, 0x28, 0x87, 0x96, 0x00,
	//0x16[1] pulse_flag HPF : Bypass ;  0x19[1:0] Deglich SW Cur : highest
	//  0x12  0x13  0x14  0x15  0x16  0x17  0x18  0x19  0x1A  0x1B
...
R912_SectType R912_IMR_Data[5] = {
	...
	{ 0, 0, 0, 0 },
	{ 0, 0, 0, 0 }
}; //  keep this array data for standby mode.
I2C_TYPE R912_I2C;
...
u32 R912_LNA_HIGH_MID[R912_TF_SIZE] = { 644000, 644000, 644000, 644000, 644000,
					500000, 500000, 500000, 500000 };
u32 R912_LNA_MID_LOW[R912_TF_SIZE] = { 388000, 388000, 348000, 348000, 348000,
				       300000, 300000, 300000, 300000 };
u32 R912_LNA_LOW_LOWEST[R912_TF_SIZE] = { 164000, 164000, 148000,
					  124000, 124000, 156000,
					  156000, 108000, 108000 };
...
u32 R912_TF_Freq_Low[R912_TF_SIZE][R912_TF_LOW_NUM] = {
	{ 320000, 304000, 272000, 240000, 232000, 216000, 192000,
	  168000 }, //164~388
	{ 320000, 304000, 272000, 240000, 232000, 216000, 192000,
	  168000 }, //164~388
	{ 256000, 240000, 232000, 224000, 216000, 192000, 168000,
	  160000 }, //148~348
	{ 256000, 240000, 232000, 192000, 160000, 152000, 144000,
	  128000 }, //124~348
	{ 264000, 240000, 192000, 184000, 176000, 152000, 144000,
	  128000 }, //124~348
	{ 280000, 248000, 232000, 216000, 192000, 176000, 168000,
	  160000 }, //156~300
	{ 280000, 248000, 232000, 216000, 192000, 176000, 168000,
	  160000 }, //156~300
...
R912_ErrCode R912_InitReg(struct r912_priv *priv,
...
R912_ErrCode R912_PLL(struct r912_priv *priv, u32 LO_Freq,
		      R912_Standard_Type R912_Standard);
R912_ErrCode R912_MUX(struct r912_priv *priv, u32 LO_KHz, u32 RF_KHz,
		      R912_Standard_Type R912_Standard);
R912_ErrCode R912_IQ(struct r912_priv *priv, R912_SectType *IQ_Pont);
R912_ErrCode R912_IQ_Tree(struct r912_priv *priv, u8 FixPot, u8 FlucPot,
			  u8 PotReg, R912_SectType *CompareTree);
R912_ErrCode R912_CompreCor(struct r912_priv *priv, R912_SectType *CorArry);
R912_ErrCode R912_CompreStep(struct r912_priv *priv, R912_SectType *StepArry,
			     u8 Pace);
...
R912_ErrCode R912_IMR_Cross(struct r912_priv *priv, R912_SectType *IQ_Pont,
			    u8 *X_Direct);
...
R912_ErrCode R912_SetStandard(struct r912_priv *priv,
			      R912_Standard_Type RT_Standard);
...
R912_Sys_Info_Type R912_Sys_Sel(struct r912_priv *priv,
				R912_Standard_Type R912_Standard);
R912_Freq_Info_Type R912_Freq_Sel(struct r912_priv *priv, u32 LO_freq,
				  u32 RF_freq,
				  R912_Standard_Type R912_Standard);
R912_SysFreq_Info_Type R912_SysFreq_Sel(struct r912_priv *priv,
					R912_Standard_Type R912_Standard,
					u32 RF_freq);

u8 R912_Filt_Cal_ADC(struct r912_priv *priv, u32 IF_Freq, u8 R912_BW,
		     u8 FilCal_Gap);

/* I2C Functions */
static int R912_Convert(int InvertNum)
{
	int ReturnNum = 0;
	int AddNum = 0x80;
	int BitNum = 0x01;
...
static int r912_i2c_readreg(struct r912_priv *priv, u8 *data, u16 size)
{
	int ret;
...
static int r912_i2c_writereg(struct r912_priv *priv, u8 *data, u16 size)
{
	int ret;
...
static bool I2C_Write_Len(struct r912_priv *priv, I2C_LEN_TYPE *R912_I2C_Len)
{
	int ret = 0;
	u8 buffer[64] = { 0 };
	u16 i = 0;
...
static bool I2C_Write(struct r912_priv *priv, I2C_TYPE *R912_I2C)
{
...
static bool I2C_Read_Len(struct r912_priv *priv, I2C_LEN_TYPE *R912_I2C_Len)
{
	int ret = 0;
	u8 buffer[64] = { 0 };
	u16 i = 0;
...
/* Lock Status */
static int R912_GetLockStatus(struct r912_priv *priv)
{
	int ret;
	I2C_LEN_TYPE i2c_para;
...
static int R912_ReadReg(struct r912_priv *priv)
{
	I2C_LEN_TYPE i2c_para;
	...
	pr_debug("R912: print Reg value start\n");
	for (i = 0; i <= index; i++) {
		pr_debug("R912: 0x%X\n", i2c_para.Data[i]);
	}
	pr_debug("R912: print Reg value end\n");
	return 0;
}
				...
				R912_Standard_Type R912_Standard)
{
	...
	case R912_DVB_C_8M_IF_5M:
		...
		R912_Sys_Info.FILT_EXT_ENA = 0x10; //R19[4]=1, ext enable
		R912_Sys_Info.FILT_EXT_WIDEST = 0x00; //R38[2]=0, ext normal
		R912_Sys_Info.FILT_EXT_POINT =
			0x02; //R38[1:0]=10, lna=max-1 & Buf 4
		break;
	...
	case R912_DTMB_IF_5M:
		R912_Sys_Info.IF_KHz = 5000;
		R912_Sys_Info.BW = 0x00; //BW=8M      R912:R19[6:5]
		R912_Sys_Info.FILT_CAL_IF =
			8650; //8825;2015-7-23           //CAL IF  8650  2015-07-06 //2015-07-10 9000
		R912_Sys_Info.HPF_COR =
			0x09; //0x0B;2015-7-23	             //R19[3:0]=9
		R912_Sys_Info.FILT_EXT_ENA = 0x10; //R19[4]=1, ext enable
		R912_Sys_Info.FILT_EXT_WIDEST = 0x00; //R38[2]=0, ext normal
	...
	if (R912_Standard < R912_ATV_SIZE) //ATV
	{
		...
		R912_Sys_Info.SWCAP_CLK =
			0x02; //8k        //AGC 500Hz map to 8k	R26[1:0]
		R912_Sys_Info.NA_PWR_DET =
			0x00; //on         						R38[7]
	}
	...
	if ((R912_Standard == R912_DVB_T2_1_7M) || (R912_Standard == R912_FM))
		...
		R912_Sys_Info.V17M = 0x00; //disable, inc DVBT2/1.7M/IF=5MHz R19[7]
...
R912_Freq_Info_Type R912_Freq_Sel(struct r912_priv *priv, u32 LO_freq,
				  u32 RF_freq, R912_Standard_Type R912_Standard)
{
	...
	/* LNA band R13[6:5] */
	if ((RF_freq >= 0) &&
	    (RF_freq < R912_LNA_LOW_LOWEST[R912_SetTfType])) //<164
		R912_Freq_Info.LNA_BAND = 0x60; // ultra low R912:R13[6:5]
	else if ((RF_freq >= R912_LNA_LOW_LOWEST[R912_SetTfType]) &&
		 (RF_freq < R912_LNA_MID_LOW[R912_SetTfType])) //164~388
		R912_Freq_Info.LNA_BAND = 0x40; //low R912:R13[6:5]
	else if ((RF_freq >= R912_LNA_MID_LOW[R912_SetTfType]) &&
		 (RF_freq < R912_LNA_HIGH_MID[R912_SetTfType])) //388~612
					...
					u32 RF_freq)
{
	...
	case R912_DVB_T2_1_7M_IF_5M:
		if ((RF_freq >= 300000) && (RF_freq <= 472000)) {
			R912_SysFreq_Info.LNA_VTH_L =
				0xA4; // LNA VTH/L=1.34/0.74     (R31=0xA4)
			R912_SysFreq_Info.MIXER_TOP =
				0x05; // MIXER TOP=10        (R36[3:0]=4'b0101)
			R912_SysFreq_Info.MIXER_VTH_L =
				0x95; // MIXER VTH/L=1.24/0.84  (R32=0x95)
			R912_SysFreq_Info.NRB_TOP =
				0xC0; // Nrb TOP=3                       (R36[7:4]=4'b1100)

		} else if ((RF_freq >= 184000) && (RF_freq <= 299000)) {
			R912_SysFreq_Info.LNA_VTH_L =
				0xA5; // LNA VTH/L=1.34/0.84     (R31=0xA5)
			R912_SysFreq_Info.MIXER_TOP =
				0x04; // MIXER TOP=11        (R36[3:0]=4'b0101)
			R912_SysFreq_Info.MIXER_VTH_L =
				0xA6; // MIXER VTH/L=1.34/0.94  (R32=0xA6)
			R912_SysFreq_Info.NRB_TOP =
				0x70; // Nrb TOP=8                       (R36[7:4]=4'b1100)
		} else {
			R912_SysFreq_Info.LNA_VTH_L =
				0xA5; // LNA VTH/L=1.34/0.84     (R31=0xA5)
			R912_SysFreq_Info.MIXER_TOP =
		...
		R912_SysFreq_Info.LNA_TOP =
			0x03; // LNA TOP=4           (R35[2:0]=3'b011)
		R912_SysFreq_Info.RF_TOP =
			0x40; // RF TOP=5                        (R34[7:5]=3'b010)
		R912_SysFreq_Info.NRB_BW =
			0xC0; // Nrb BW=lowest                  (R35[7:6]=2'b11)
		break;
	...
	case R912_J83B_IF_5M:
		if (RF_freq < 165000) {
			R912_SysFreq_Info.LNA_TOP =
				0x02; // LNA TOP=5                    (R35[2:0]=3'b010)
			R912_SysFreq_Info.LNA_VTH_L =
				0x94; // LNA VTH/L=1.24/0.74     (R31=0x94)
			R912_SysFreq_Info.RF_TOP =
				0x80; // RF TOP=3                        (R34[7:5]=3'b100)
			R912_SysFreq_Info.NRB_TOP =
				0x90; // Nrb TOP=6                       (R36[7:4]=4'b1001)   //R912:R36[7:4]
			...
			R912_SysFreq_Info.RF_TOP =
				0x80; // RF TOP=3                        (R34[7:5]=3'b100)
			R912_SysFreq_Info.NRB_TOP =
				0x90; // Nrb TOP=6                       (R36[7:4]=4'b1001)
			...
			R912_SysFreq_Info.LNA_TOP =
				0x04; // LNA TOP=3                    (R35[2:0]=3'b100)
			R912_SysFreq_Info.LNA_VTH_L =
				0x93; // LNA VTH/L=1.24/0.64     (R31=0x93)
			R912_SysFreq_Info.RF_TOP =
				0xA0; // RF TOP=2                        (R34[7:5]=3'b101)
			R912_SysFreq_Info.NRB_TOP =
				0x20; // Nrb TOP=13                       (R36[7:4]=4'b0010)
		} else {
			R912_SysFreq_Info.LNA_TOP =
				0x04; // LNA TOP=3                    (R35[2:0]=3'b100)
			R912_SysFreq_Info.LNA_VTH_L =
				0x83; // LNA VTH/L=1.14/0.64     (R31=0x83)
			R912_SysFreq_Info.RF_TOP =
				0xA0; // RF TOP=2                        (R34[7:5]=3'b101)
			R912_SysFreq_Info.NRB_TOP =
				0x20; // Nrb TOP=13                       (R36[7:4]=4'b0010)
		...
		R912_SysFreq_Info.MIXER_TOP =
			0x05; // MIXER TOP=10               (R36[3:0]=4'b0100)
		R912_SysFreq_Info.MIXER_VTH_L =
			0x95; // MIXER VTH/L=1.24/0.84  (R32=0x95)
		R912_SysFreq_Info.NRB_BW =
			0xC0; // Nrb BW=lowest                  (R35[7:6]=2'b11)
	...
	case R912_ISDB_T_IF_5M:
		...
		R912_SysFreq_Info.MIXER_VTH_L =
			0x95; // MIXER VTH/L=1.24/0.84  (R32=0x95)
		R912_SysFreq_Info.RF_TOP =
			0x60; // RF TOP=4                        (R34[7:5]=3'b011)
		//R912_SysFreq_Info.NRB_TOP=0x20;            // Nrb TOP=13                       (R36[7:4]=4'b0010)
		R912_SysFreq_Info.NRB_TOP =
			0xB0; // Nrb TOP=4                       (R36[7:4]=4'b1011)
		R912_SysFreq_Info.NRB_BW =
			0xC0; // Nrb BW=lowest                  (R35[7:6]=2'b11)
	...
	case R912_DTMB_IF_5M:
		...
		R912_SysFreq_Info.RF_TOP =
			0x60; // RF TOP=4                        (R34[7:5]=3'b011)
		R912_SysFreq_Info.NRB_TOP =
			0xA0; // Nrb TOP=5                       (R36[7:4]=4'b1010)
		R912_SysFreq_Info.NRB_BW =
			0xC0; // Nrb BW=lowest                  (R35[7:6]=2'b11)
		break;
	...
	case R912_ATSC_IF_5M:
		if (R912_DetectTfType == R912_UL_USING_BEAD) {
			if (RF_freq < 88000) {
				...
				R912_SysFreq_Info.LNA_VTH_L =
					0xA5; // LNA VTH/L=1.34/0.84     (R31=0xA5)
				R912_SysFreq_Info.RF_TOP =
					0xC0; // RF TOP=1                        (R34[7:5]=3'b110)
				R912_SysFreq_Info.NRB_TOP =
					0x30; // Nrb TOP=12                    (R36[7:4]=4'b0011)
				R912_SysFreq_Info.NRB_BW =
					0xC0; // Nrb BW=lowest                  (R35[7:6]=2'b11)
				...
				R912_SysFreq_Info.LNA_TOP =
					0x01; // LNA TOP=6                    (R35[2:0]=3'b001)
				R912_SysFreq_Info.LNA_VTH_L =
					0xB6; // LNA VTH/L=1.44/0.94     (R31=0xB6)
				R912_SysFreq_Info.RF_TOP =
					0x60; // RF TOP=4                        (R34[7:5]=3'b011)
				R912_SysFreq_Info.NRB_TOP =
					0x90; // Nrb TOP=6                      (R36[7:4]=4'b1001)
		...
		R912_SysFreq_Info.MIXER_TOP =
			0x04; // MIXER TOP=11               (R36[3:0]=4'b0100)
		R912_SysFreq_Info.MIXER_VTH_L =
			0xB7; // MIXER VTH/L=1.44/1.04  (R32=0xB7)
	...
	R912_SysFreq_Info.RF_FAST_DISCHARGE =
		0x00; //0 	 R912:R46[3:1]=3'b000
	R912_SysFreq_Info.RF_SLOW_DISCHARGE = 0x80; //4   R912:R22[7:5]=2'b100
	R912_SysFreq_Info.RFPD_PLUSE_ENA = 0x10; //1   R912:R38[4]=1

	R912_SysFreq_Info.LNA_FAST_DISCHARGE =
		0x0A; //10  R912:R43[4:0]=5'b01010
	...
	case R912_DVB_T2_1_7M_IF_5M:
		if (RF_freq >= 662000 && RF_freq <= 670000) {
			R912_SysFreq_Info.NRB_TOP =
				0xB0; // Nrb TOP=4                       (R36[7:4]=4'b1011)
			R912_SysFreq_Info.RF_SLOW_DISCHARGE =
				0x20; //1   R912:R22[7:5]=2'b001
			R912_SysFreq_Info.AGC_CLK = 0x18; //60Hz   R26[4:2]
		} else {
			R912_SysFreq_Info.RF_SLOW_DISCHARGE =
				0x80; //4   R912:R22[7:5]=2'b100
...
R912_ErrCode R912_Init(struct r912_priv *priv)
{
	...
	if (R912_Initial_done_flag == R912_FALSE) {
		...
		if (R912_IMR_done_flag == R912_FALSE) {
			...
			if (R912_InitReg(priv, R912_STD_SIZE) !=
			    RT_Success) //write initial reg before doing IMR Cal
				return RT_Fail;

			if (R912_Cal_Prepare(priv, R912_IMR_CAL) != RT_Success)
				return RT_Fail;

			if (R912_IMR(priv, 3, R912_TRUE) !=
			    RT_Success) //Full K node 3
				return RT_Fail;
			...
			if (R912_IMR(priv, 4, R912_TRUE) !=
			    RT_Success) //Full K node 4
				return RT_Fail;
			  ...
			  R912_Standard_Type R912_Standard)
{
	...
	if (R912_Standard != R912_DVB_S) {
		for (InitArrayCunt = 0; InitArrayCunt < R912_REG_NUM;
		     InitArrayCunt++) {
			R912_I2C_Len.Data[InitArrayCunt] =
				R912_iniArray_hybrid[InitArrayCunt];
			R912_Array[InitArrayCunt] =
				R912_iniArray_hybrid[InitArrayCunt];
		}
	} else {
		for (InitArrayCunt = 0; InitArrayCunt < R912_REG_NUM;
		     InitArrayCunt++) {
			R912_I2C_Len.Data[InitArrayCunt] =
				R912_iniArray_dvbs[InitArrayCunt];
			R912_Array[InitArrayCunt] =
				R912_iniArray_dvbs[InitArrayCunt];
		}
	}
...
R912_ErrCode R912_TF_Check(struct r912_priv *priv)
{
	...
	R912_Array[31] = (R912_Array[31] & 0x03) | 0x80 |
			 (divnum_ring << 2); //pre div=6 & div_num
	...
	R912_Array[25] = (R912_Array[25] & 0x00) |
			 0x8B; //out div=8, RF poly=low band, power=min_lp
	if (RingVCO >= 3200000)
		R912_Array[25] =
			(R912_Array[25] & 0xDF); //vco_band=high, R25[5]=0
	else
		R912_Array[25] =
			(R912_Array[25] | 0x20); //vco_band=low, R25[5]=1
	...
	//Must do before PLL()
	if (R912_MUX(priv, RingFreq + 5000, RingFreq, R912_STD_SIZE) !=
	    RT_Success) //FilCal MUX (LO_Freq, RF_Freq)
		return RT_Fail;

	//Set PLL
	if (R912_PLL(priv, (RingFreq + 5000), R912_STD_SIZE) !=
	    RT_Success) //FilCal PLL
		return RT_Fail;
...
R912_ErrCode R912_Xtal_Check(struct r912_priv *priv)
{
	...
	for (i = 0; i < XTAL_CHECK_SIZE; i++) //from weak to strong
	{
		//set power
		if (i ==
		    XTAL_LARGE_STRONG) // Xtal_pow0=highest(00)  R912:R23[6:5]   ;    Xtal_pow1=low(1)  R912:R23[7]
		{
			R912_Array[15] = (R912_Array[15] & 0x1F) | 0X80;
			R912_Array[19] = (R912_Array[19] & 0xFE) |
					 0x01; //| 0x00;  //LARGE Gm(1)
		} else if (i <=
			   XTAL_SMALL_HIGHEST) //Xtal_pow1=high(0)  R912:R23[7]
		{
			R912_Array[15] = (R912_Array[15] & 0x1F) |
					 ((u8)(XTAL_SMALL_HIGHEST - i) << 5);
			R912_Array[19] = (R912_Array[19] & 0xFE) |
					 0x00; //| 0x00;  //SMALL Gm(0)
		...
		if (((R912_I2C_Len.Data[2] & 0x40) == 0x40) &&
		    ((R912_I2C_Len.Data[2] & 0x3F) <= (vco_check_bank + 6)) &&
		    ((R912_I2C_Len.Data[2] & 0x3F) >= (vco_check_bank - 6))) {
			R912_Xtal_Pwr_tmp = i;
			break;
		}
...
R912_ErrCode R912_Cal_Prepare(struct r912_priv *priv, u8 u1CalFlag)
{
	...
	case R912_IMR_CAL:
		...
		Cal_Info.LNA_POWER =
			0x80; //LNA power OFF,RF_Buf pwr off  //  R912:R8[7]
		//Cal_Info.LNA_POWER = 0x00;				//LNA need on
		Cal_Info.TF_CAL =
			0x00; // TF cal OFF, -6dB	OFF   // R912:R14[6:5]
		Cal_Info.MIXER_AMP_GAIN =
			0x08; //manual +8				  // R912:R15[4:0]
		Cal_Info.MIXER_BUFFER_GAIN =
			0x10; //manual min(0)			  // R912:R34[4:0]
		Cal_Info.LNA_GAIN =
			0x9F; //manual: max		//  R912:R13[7:0]
	...
	R912_Array[4] =
		(R912_Array[4] & 0xDF) |
		Cal_Info.RFBUF_OUT; //  R912:R12[5]  12-8=4  12(0x0C) is addr ; [4] is data

	//RF_Buf Power
	//R912_I2C.RegAddr = 0x09;
	R912_Array[1] =
		(R912_Array[1] & 0xFB) |
		Cal_Info.RFBUF_POWER; //  R912:R12[5]  12-8=4  12(0x0C) is addr ; [4] is data
	...
	R912_Array[0] =
		(R912_Array[0] & 0x7F) |
		Cal_Info.LNA_POWER; //  R912:R8[7]  8-8=0  8(0x08) is addr ; [0] is data
	...
	R912_Array[6] =
		(R912_Array[6] & 0x9F) |
		Cal_Info.TF_CAL; // R912:R14[6:5]  14-8=6  14(0x0E) is addr ; [6] is data

	//LNA gain
	//R912_I2C.RegAddr = 0x0D;
	R912_Array[5] =
		(R912_Array[5] & 0x60) |
		Cal_Info.LNA_GAIN; // R912:R13[7:0]  13-8=5  13(0x0D) is addr ; [5] is data

	//Mixer Amp Gain
	//R912_I2C.RegAddr = 0x0F;
	R912_Array[7] =
		(R912_Array[7] & 0xE0) |
		Cal_Info.MIXER_AMP_GAIN; // R912:R15[4:0]  15-8=7  15(0x0F) is addr ; [7] is data
...
R912_ErrCode R912_IMR(struct r912_priv *priv, u8 IMR_MEM, BOOL IM_Flag)
{
	...
	R912_Array[31] &=
		0x3F; //clear ring_div1, R24[7:6]	//R912:R39[7:6]  39-8=31  39(0x27) is addr ; [31] is data
	R912_Array[25] &=
		0xFC; //clear ring_div2, R25[1:0]	//R912:R33[1:0]  33-8=25  33(0x21) is addr ; [25] is data
	...
	if (RingVCO >= 3200000) {
		R912_Array[25] &=
			0xDF; //clear vco_band, R25[5]		//R912:R33[5]    33-8=25  33(0x21) is addr ; [25] is data
	} else {
		R912_Array[25] |=
			0x20; //clear vco_band, R25[5]		//R912:R33[5]    33-8=25  33(0x21) is addr ; [25] is data
	}
	...
	if ((RingFreq >= 0) &&
	    ((RingFreq < R912_RING_POWER_FREQ_LOW) ||
	     (RingFreq > R912_RING_POWER_FREQ_HIGH))) //R912:R33[3:2]
		R912_Array[25] = (R912_Array[25] & 0xF3) |
				 0x08; //R25[3:2]=2'b10; min_lp
	else
		R912_Array[25] = (R912_Array[25] & 0xF3) |
				 0x00; //R25[3:2]=2'b00; min

	//Must do MUX before PLL()
	if (R912_MUX(priv, RingFreq - R912_IMR_IF, RingFreq, R912_STD_SIZE) !=
	    RT_Success) //IMR MUX (LO, RF)
		return RT_Fail;

	if (R912_PLL(priv, (RingFreq - R912_IMR_IF), R912_STD_SIZE) !=
	    RT_Success) //IMR PLL
		return RT_Fail;
		      ...
		      R912_Standard_Type R912_Standard)
{
	...
	u8 u1PulseFlag = 0;
	u8 u1SPulseFlag = 0;
	u8 cp_cur_24[50] = { 5, 5, 5, 5, 5, 4, 0, 4, 5, 4, 5, 3, 5, 3, 3, 0, 3,
			     0, 0, 0, 0, 0, 0, 0, 0, 0, 5, 0, 3, 0, 4, 0, 2, 0,
			     4, 0, 3, 5, 1, 5, 0, 5, 2, 5, 3, 5, 3, 5, 1, 5 };

	u8 cp_cur_27[38] = { 0, 0, 0, 0, 0, 0, 1, 5, 1, 5, 4, 5, 4,
			     0, 4, 0, 4, 0, 0, 0, 0, 0, 3, 1, 4, 1,
			     1, 1, 4, 0, 2, 0, 4, 5, 2, 5, 3, 2 };

	//112MHz to 176MHz CP Set Value
	u8 cp_cur_16_low[5] = { 5, 2, 3, 1, 3 };
	//304MHz to 496MHz CP Set Value
	u8 cp_cur_16_mid[25] = { 2, 0, 4, 0, 3, 0, 1, 0, 4, 0, 2, 0, 3,
				 0, 4, 0, 1, 5, 1, 5, 1, 5, 3, 5, 4 };
	...
	//R912_I2C.RegAddr = 0x1B;
	if ((R912_Xtal == 16000) ||
	    (R912_Xtal == 27000)) // Xtal cap =16pF  (16MHz)
	...
	if (R912_Xtal == 16000) {
		...
		if (R912_Standard < R912_ATV_SIZE) //ATV
		{
			...
			if ((u2XalDivJudge == 6) || (u2XalDivJudge == 14) ||
			    (u2XalDivJudge == 16) || (u2XalDivJudge == 18) ||
			    (u2XalDivJudge == 20))
			...
			if (LO_Freq <
			    (160000 +
			     R912_IF_HIGH)) //  R912:R21[7]  21-8=13  21(0x15) is addr ; [13] is data	// R912:R21[7]
			{
				CP_OFFSET = 0x80; //30u,   [2]=1
			} else if ((u2XalDivJudge >= 38) &&
				   (u2XalDivJudge < 63)) {
			...
			if (LO_Freq < (48000 + R912_IF_HIGH))
				...
				CP_CUR = (0x00 |
					  (cp_cur_16_mid[(u2XalDivJudge - 38)]
					   << 4));
	...
	} else if (R912_Xtal == 24000) //24MHz
	{
		...
		if (R912_Standard < R912_ATV_SIZE) //ATV
		{
			...
			if ((u2XalDivJudge == 6) || (u2XalDivJudge == 10) ||
			    (u2XalDivJudge == 12))
				//SDM_RES = 0x18;    //400R, R27[4:3]=11
				SDM_RES = 0x10; //200R, R27[4:3]=10
			else
				SDM_RES = 0x00; //short, R27[4:3]=00
			//offset
			if ((LO_Freq < (160000 + R912_IF_HIGH)) ||
			    (u2XalDivJudge ==
			     16)) //R912:R21[7]  21-8=13  21(0x15) is addr ; [13] is data
			{
				CP_OFFSET = 0x80; //30u,   [2]=1
			} else if ((u2XalDivJudge >= 25) &&
				   (u2XalDivJudge < 50)) {
				if ((u2XalDivJudge % 2) == 1) //odd
					CP_OFFSET = 0x00; //0u,   [2]=0
				else
	...
	} else //27MHz
	{
		...
		if (R912_Standard < R912_ATV_SIZE) //ATV
		{
			...
			if (LO_Freq <
			    (189000 +
			     R912_IF_HIGH)) //R912:R21[7]  21-8=13  21(0x15) is addr ; [13] is data
			{
				CP_OFFSET = 0x80; //30u,   [2]=1
			} else if ((u2XalDivJudge >= 22) &&
				   (u2XalDivJudge < 37)) {
				if ((u2XalDivJudge % 2) == 1) //odd
					CP_OFFSET = 0x00; //0u,   [2]=0
				else
	...
	if (R912_Initial_done_flag == R912_TRUE) {
		if (R912_Xtal_Pwr < XTAL_SMALL_HIGH) {
			...
			XTAL_POW0 = ((u8)(XTAL_SMALL_HIGHEST - R912_Xtal_Pwr)
				     << 5); //R912:R23[6:5]
			XTAL_GM = 0x00; //SMALL(0),   R27[0]=0
	...
	//Divider
	while (MixDiv <= 64) {
		if (((LO_Freq * MixDiv) >= VCO_Min) &&
		    ((LO_Freq * MixDiv) < VCO_Max)) {
			DivBuf = MixDiv;
			while (DivBuf > 2) {
				DivBuf = DivBuf >> 1;
				DivNum++;
	...
	*/
	...
	//Xtal Div
	if ((R912_Standard == R912_STD_SIZE) ||
	    (R912_Standard == R912_DVB_S)) //for cal and Satellite
	{
		R912_XtalDiv = XTAL_DIV1;
		R912_Array[16] = R912_Array[16] & 0xFB; //b2=0  // R912:R24[2]
		PLL_Ref = R912_Xtal;
	} else if ((R912_Xtal == 16000) ||
		   (R912_Xtal == 24000)) //16MHz, 24MHz (ATV_DTV_Standard)
	{
...
*/
	...
	else if (VCO_Fra > PLL_Ref * 127 / 64) //2*PLL_Ref*127/128
	{
		VCO_Fra = 0;
		Nint++;
	} else if ((VCO_Fra > PLL_Ref * 127 / 128) &&
		   (VCO_Fra <
		    PLL_Ref)) //> 2*PLL_Ref*127/256,  < 2*PLL_Ref*128/256
		VCO_Fra = PLL_Ref * 127 / 128; // VCO_Fra = 2*PLL_Ref*127/256
	else if ((VCO_Fra > PLL_Ref) &&
		 (VCO_Fra <
		  PLL_Ref * 129 /
			  128)) //> 2*PLL_Ref*128/256,  < 2*PLL_Ref*129/256
	...
	for (VCO_current_trial = 0; VCO_current_trial < 3;
	     VCO_current_trial++) {
		...
		if ((R912_I2C_Len.Data[2] & 0x40) == 0x00) {
			...
			R912_Array[18] = (R912_Array[18] & 0x1F) |
					 ((2 - VCO_current_trial)
					  << 5); //increase VCO current
			R912_I2C.Data = R912_Array[18];
			if (I2C_Write(priv, &R912_I2C) != RT_Success)
				return RT_Fail;
		} else {
	...
	if (VCO_current_trial >= 2) {
		...
		if ((R912_I2C_Len.Data[2] & 0x40) == 0x00) {
			...
			R912_I2C.RegAddr =
				0x17; // XTAL_POW0:R23[6:5] ;  XTAL_POW1:R23[7]
			R912_Array[15] = ((R912_Array[15] & 0x1F) | XTAL_POW0) |
					 XTAL_POW1;
			R912_I2C.Data = R912_Array[15];
			if (I2C_Write(priv, &R912_I2C) != RT_Success)
				return RT_Fail;
		      ...
		      R912_Standard_Type R912_Standard)
{
	...
	if (R912_IMR_done_flag == R912_TRUE) {
		Reg08_IMR_Gain = R912_IMR_Data[Freq_Info1.IMR_MEM].Gain_X &
				 0x3F;
		Reg09_IMR_Phase = R912_IMR_Data[Freq_Info1.IMR_MEM].Phase_Y &
				  0x3F;
		Reg03_IMR_Iqcap = R912_IMR_Data[Freq_Info1.IMR_MEM].Iqcap &
				  0x03;
	} else {
		Reg08_IMR_Gain = 0;
		Reg09_IMR_Phase = 0;
...
R912_ErrCode R912_SetTF(struct r912_priv *priv, u32 u4FreqKHz, u8 u1TfType)
{
	...
	if ((u4FreqKHz >= 0) &&
	    (u4FreqKHz < R912_LNA_LOW_LOWEST[R912_SetTfType])) //Ultra Low
	{
		//u1TFCalNum = R912_TF_LOWEST_NUM;
		while ((u4FreqKHz <
			R912_TF_Freq_Lowest[u1TfType][u1FreqCount]) &&
		       (u1FreqCount < R912_TF_LOWEST_NUM)) {
			u1FreqCount++;
		}
		...
		if (u1FreqCount == 0) {
			...
			R912_TF = R912_TF_Result_Lowest[u1TfType]
						       [R912_TF_LOWEST_NUM - 1];
		} else {
			u1TF_Set_Result1 =
				R912_TF_Result_Lowest[u1TfType][u1FreqCount - 1];
			u1TF_Set_Result2 =
				R912_TF_Result_Lowest[u1TfType][u1FreqCount];
			u4Freq1 =
				R912_TF_Freq_Lowest[u1TfType][u1FreqCount - 1];
			...
			u1TF_tmp1 = ((u1TF_Set_Result1 & 0x40) >> 2) * 3 +
				    (u1TF_Set_Result1 & 0x3F); //b6 is 3xb4
			u1TF_tmp2 = ((u1TF_Set_Result2 & 0x40) >> 2) * 3 +
				    (u1TF_Set_Result2 & 0x3F);
			u4Ratio = (u4Freq1 - u4FreqKHz) * 100 /
				  (u4Freq1 - u4Freq2);
			R912_TF = u1TF_tmp1 +
				  (u8)((u1TF_tmp2 - u1TF_tmp1) * u4Ratio / 100);
			if (R912_TF >= 0x40)
				R912_TF = (R912_TF + 0x10);
		}
	} else if ((u4FreqKHz >= R912_LNA_LOW_LOWEST[R912_SetTfType]) &&
		   (u4FreqKHz < R912_LNA_MID_LOW[R912_SetTfType])) //Low
	{
		//u1TFCalNum = R912_TF_LOW_NUM;
		while ((u4FreqKHz < R912_TF_Freq_Low[u1TfType][u1FreqCount]) &&
		       (u1FreqCount < R912_TF_LOW_NUM)) {
			u1FreqCount++;
		}
		...
		if (u1FreqCount == 0) {
			...
			R912_TF = R912_TF_Result_Low[u1TfType]
						    [R912_TF_LOW_NUM - 1];
		} else {
			u1TF_Set_Result1 =
				R912_TF_Result_Low[u1TfType][u1FreqCount - 1];
			u1TF_Set_Result2 =
				R912_TF_Result_Low[u1TfType][u1FreqCount];
			...
			u1TF_tmp1 = ((u1TF_Set_Result1 & 0x40) >> 2) +
				    (u1TF_Set_Result1 & 0x3F); //b6 is 1xb4
			u1TF_tmp2 = ((u1TF_Set_Result2 & 0x40) >> 2) +
				    (u1TF_Set_Result2 & 0x3F);
			u4Ratio = (u4Freq1 - u4FreqKHz) * 100 /
				  (u4Freq1 - u4Freq2);
			R912_TF = u1TF_tmp1 +
	...
	} else if ((u4FreqKHz >= R912_LNA_MID_LOW[R912_SetTfType]) &&
		   (u4FreqKHz < R912_LNA_HIGH_MID[R912_SetTfType])) //Mid
	{
		//u1TFCalNum = R912_TF_MID_NUM;
		while ((u4FreqKHz < R912_TF_Freq_Mid[u1TfType][u1FreqCount]) &&
		       (u1FreqCount < R912_TF_MID_NUM)) {
			u1FreqCount++;
		}
		...
		if (u1FreqCount == 0) {
			...
			R912_TF = R912_TF_Result_Mid[u1TfType]
						    [R912_TF_MID_NUM - 1];
		} else {
			u1TF_Set_Result1 =
				R912_TF_Result_Mid[u1TfType][u1FreqCount - 1];
			u1TF_Set_Result2 =
				R912_TF_Result_Mid[u1TfType][u1FreqCount];
			u4Freq1 = R912_TF_Freq_Mid[u1TfType][u1FreqCount - 1];
			u4Freq2 = R912_TF_Freq_Mid[u1TfType][u1FreqCount];
			u4Ratio = (u4Freq1 - u4FreqKHz) * 100 /
				  (u4Freq1 - u4Freq2);
			R912_TF = u1TF_Set_Result1 +
				  (u8)((u1TF_Set_Result2 - u1TF_Set_Result1) *
				       u4Ratio / 100);
		}
	} else //HIGH
	{
		//u1TFCalNum = R912_TF_HIGH_NUM;
		while ((u4FreqKHz < R912_TF_Freq_High[u1TfType][u1FreqCount]) &&
		       (u1FreqCount < R912_TF_HIGH_NUM)) {
			u1FreqCount++;
		}
		...
		if (u1FreqCount == 0) {
			...
			R912_TF = R912_TF_Result_High[u1TfType]
						     [R912_TF_HIGH_NUM - 1];
		} else {
			u1TF_Set_Result1 =
				R912_TF_Result_High[u1TfType][u1FreqCount - 1];
			u1TF_Set_Result2 =
				R912_TF_Result_High[u1TfType][u1FreqCount];
...
R912_ErrCode R912_IQ(struct r912_priv *priv, R912_SectType *IQ_Pont)
{
	...
	if (X_Direction == 1) {
		...
		if (R912_CompreStep(priv, &Compare_IQ[0], 0x10) !=
		    RT_Success) //X
			return RT_Fail;
	} else {
		//compare and find min of 3 points. determine I/Q direction
		...
		//increase step to find min value of this direction
		if (R912_CompreStep(priv, &Compare_IQ[0], 0x11) !=
		    RT_Success) //Y
			return RT_Fail;
	}
	...
	//Another direction
	if (X_Direction == 1) {
		if (R912_IQ_Tree(priv, Compare_IQ[0].Gain_X,
				 Compare_IQ[0].Phase_Y, 0x10,
				 &Compare_IQ[0]) != RT_Success) //Y
			return RT_Fail;
		...
		if (R912_IQ_Tree(priv, Compare_IQ[0].Phase_Y,
				 Compare_IQ[0].Gain_X, 0x11,
				 &Compare_IQ[0]) != RT_Success) //X
			return RT_Fail;
...
/*
 * Purpose: Record IMC results by input gain/phase location then ajust gain
 *          or phase positive 1 step and negtive 1 step, both record results
			  ...
			  u8 PotReg, R912_SectType *CompareTree)
{
	...
	for (TreeCunt = 0; TreeCunt < TreeTimes; TreeCunt++) {
		...
		if (R912_Muti_Read(priv, &CompareTree[TreeCunt].Value) !=
		    RT_Success)
			return RT_Fail;
		...
		{
			if ((FlucPot & 0x1F) ==
			    1) //if absolute location is 1, change I/Q direction
			{
				if (FlucPot &
				    0x20) //b[5]:I/Q selection. 0:Q-path, 1:I-path
				{
...
R912_ErrCode R912_CompreCor(struct r912_priv *priv, R912_SectType *CorArry)
{
	...
	for (CompCunt = 3; CompCunt > 0; CompCunt--) {
		if (CorArry[0].Value >
		    CorArry[CompCunt - 1]
			    .Value) //compare IMC result [0][1][2], find min value
		{
			CorTemp = CorArry[0];
			     ...
			     u8 Pace)
{
	...
	while (((StepTemp.Gain_X & 0x1F) < R912_IMR_TRIAL) &&
	       ((StepTemp.Phase_Y & 0x1F) < R912_IMR_TRIAL)) {
		if (Pace == 0x10)
			StepTemp.Gain_X++;
		else
			StepTemp.Phase_Y++;
		...
		} else if ((StepTemp.Value - 2 * ADC_READ_COUNT) >
			   StepArry[0].Value) {
			break;
		}
...
R912_ErrCode R912_Section(struct r912_priv *priv, R912_SectType *IQ_Pont)
{
	...
	if ((IQ_Pont->Gain_X & 0x1F) == 0x00) {
		Compare_IQ[0].Gain_X =
			((IQ_Pont->Gain_X) & 0xDF) + 1; //Q-path, Gain=1
	} else {
		Compare_IQ[0].Gain_X = IQ_Pont->Gain_X - 1; //left point
	}
	Compare_IQ[0].Phase_Y = IQ_Pont->Phase_Y;

	if (R912_IQ_Tree(priv, Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y,
			 0x10, &Compare_IQ[0]) != RT_Success) // y-direction
		return RT_Fail;
	...
	if ((IQ_Pont->Gain_X & 0x1F) == 0x00)
		Compare_IQ[0].Gain_X =
			((IQ_Pont->Gain_X) | 0x20) + 1; //I-path, Gain=1
	else
...
R912_ErrCode R912_IMR_Cross(struct r912_priv *priv, R912_SectType *IQ_Pont,
			    u8 *X_Direct)
{
	R912_SectType Compare_Cross
		[9]; //(0,0)(0,Q-1)(0,I-1)(Q-1,0)(I-1,0)+(0,Q-2)(0,I-2)(Q-2,0)(I-2,0)
	...
	for (CrossCount = 0; CrossCount < 9; CrossCount++) {
		...
		if (R912_Muti_Read(priv, &Compare_Cross[CrossCount].Value) !=
		    RT_Success)
			return RT_Fail;
		...
		if (Compare_Cross[CrossCount].Value < Compare_Temp.Value) {
			...
			Compare_Temp.Phase_Y =
				Compare_Cross[CrossCount].Phase_Y;
		}
	} //end for loop

	if (((Compare_Temp.Phase_Y & 0x3F) == 0x01) ||
	    (Compare_Temp.Phase_Y & 0x3F) == 0x02) //phase Q1 or Q2
	{
		*X_Direct = (u8)0;
	...
	} else if (((Compare_Temp.Phase_Y & 0x3F) == 0x21) ||
		   (Compare_Temp.Phase_Y & 0x3F) == 0x22) //phase I1 or I2
	{
		*X_Direct = (u8)0;
		IQ_Pont[0].Gain_X = Compare_Cross[0].Gain_X; //0
		IQ_Pont[0].Phase_Y = Compare_Cross[0].Phase_Y; //0
	...
	} else if (((Compare_Temp.Gain_X & 0x3F) == 0x01) ||
		   (Compare_Temp.Gain_X & 0x3F) == 0x02) //gain Q1 or Q2
	{
		*X_Direct = (u8)1;
		IQ_Pont[0].Gain_X = Compare_Cross[0].Gain_X; //0
		IQ_Pont[0].Phase_Y = Compare_Cross[0].Phase_Y; //0
	...
	} else if (((Compare_Temp.Gain_X & 0x3F) == 0x21) ||
		   (Compare_Temp.Gain_X & 0x3F) == 0x22) //gain I1 or I2
	{
		*X_Direct = (u8)1;
		IQ_Pont[0].Gain_X = Compare_Cross[0].Gain_X; //0
		IQ_Pont[0].Phase_Y = Compare_Cross[0].Phase_Y; //0
			      ...
			      R912_Standard_Type RT_Standard)
{
	...
	if (R912_Fil_Cal_flag[RT_Standard] == R912_FALSE) {
		R912_Fil_Cal_code[RT_Standard] = R912_Filt_Cal_ADC(
			priv, Sys_Info1.FILT_CAL_IF, Sys_Info1.BW, u1FilCalGap);
	...
	R912_Array[10] = (R912_Array[10] & 0xF0) |
			 R912_Fil_Cal_code[RT_Standard];
	R912_I2C.Data = R912_Array[10];
	if (I2C_Write(priv, &R912_I2C) != RT_Success)
		return RT_Fail;
	...
	R912_Array[11] = (R912_Array[11] & 0x70) | Sys_Info1.HPF_COR |
			 Sys_Info1.V17M;
	R912_I2C.Data = R912_Array[11];
	if (I2C_Write(priv, &R912_I2C) != RT_Success)
		return RT_Fail;
	...
	R912_Array[30] = (R912_Array[30] & 0xF8) | 0x04 |
			 Sys_Info1.FILT_EXT_POINT; //ext both HPF/LPF
	R912_I2C.Data = R912_Array[30];
	if (I2C_Write(priv, &R912_I2C) != RT_Success)
		return RT_Fail;
	/*
		     ...
		     u8 FilCal_Gap)
{
	...
	//Must do before PLL()
	if (R912_MUX(priv, u4RingFreq + IF_Freq, u4RingFreq, R912_STD_SIZE) !=
	    RT_Success) //FilCal MUX (LO_Freq, RF_Freq)
		return RT_Fail;

	//Set PLL
	if (R912_PLL(priv, (u4RingFreq + IF_Freq), R912_STD_SIZE) !=
	    RT_Success) //FilCal PLL
		return RT_Fail;

	//-----below must set after R912_MUX()-------//
	//Set LNA TF for RF=72MHz. no use
	if ((R912_SetTfType == R912_TF_NARROW) ||
	    (R912_SetTfType == R912_TF_NARROW_LIN)) //UL use 270n setting
	{
	...
	for (i = initial_cnt; i < 3; i++) {
		...
		if (u1FilterCalValuePre >
		    (u1FilterCalValue + 8)) //suitable BW found
			break;
	}
	...
	for (u1FilterCode = 0; u1FilterCode < 16; u1FilterCode++) {
		...
		if ((u1FilterCalValue + FilCal_Gap * ADC_READ_COUNT) <
		    u1FilterCalValuePre) {
...
R912_ErrCode R912_SetFrequency(struct r912_priv *priv, R912_Set_Info R912_INFO)
{
	...
	if (R912_INFO.R912_Standard != R912_DVB_S) {
		...
		if ((R912_INFO.RF_KHz < 40000) ||
		    (R912_INFO.RF_KHz > 1002000)) {
			return RT_Fail;
		}
	} else {
		// Check Input Frequency Range
		if ((R912_INFO.RF_KHz < 40000) ||
		    (R912_INFO.RF_KHz > 2400000)) {
			return RT_Fail;
		}
	}
	...
	if (R912_MUX(priv, LO_KHz, R912_INFO.RF_KHz, R912_INFO.R912_Standard) !=
	    RT_Success) //normal MUX
		return RT_Fail;

	//Set PLL
	if (R912_PLL(priv, LO_KHz, R912_INFO.R912_Standard) !=
	    RT_Success) //noraml PLL
		return RT_Fail;
	...
	if ((R912_INFO.RF_KHz > 700000) &&
	    (R912_INFO.R912_Standard < R912_ATV_SIZE)) {
		R912_I2C.RegAddr = 0x08;
		R912_Array[0] = (R912_Array[0] & 0x80) | 0x00;
		R912_I2C.Data = R912_Array[0];
		if (I2C_Write(priv, &R912_I2C) != RT_Success)
	...
	//medQctrl 1.5K
	if ((R912_INFO.RF_KHz >= 300000) &&
	    (R912_INFO.RF_KHz <= 472000)) //<473MHz and >299MHz
	...
	//3~6 shrink
	if ((R912_INFO.RF_KHz >= 300000) &&
	    (R912_INFO.RF_KHz <= 550000)) //<551MHz and >299MHz
	...
	SysFreq_Info1 = R912_SysFreq_Sel(priv, R912_INFO.R912_Standard,
					 R912_INFO.RF_KHz);
	...
	R912_Array[38] = (R912_Array[38] & 0xF1) |
			 SysFreq_Info1.RF_FAST_DISCHARGE;

	//R912_I2C.RegAddr = 0x16;	//4   R912:R22[7:5]=2'b010
	R912_Array[14] = (R912_Array[14] & 0x1F) |
			 SysFreq_Info1.RF_SLOW_DISCHARGE;
	...
	R912_Array[35] = (R912_Array[35] & 0xE0) |
			 SysFreq_Info1.LNA_FAST_DISCHARGE;

	//R912_I2C.RegAddr = 0x16;	//0  R912:R22[4:2]=3'b000
	R912_Array[14] = (R912_Array[14] & 0xE3) |
			 SysFreq_Info1.LNA_SLOW_DISCHARGE;
	...
	if (R912_INFO.R912_Standard < R912_ATV_SIZE) // ATV
	{
		if (R912_XtalDiv == XTAL_DIV1) {
			...
			R912_Array[18] = (R912_Array[18] & 0xE3) |
					 0x14; //[4:2]=101
		} else {
			//AGC CLK to 1khz
			R912_Array[18] = (R912_Array[18] & 0xE3) |
		...
		case R912_DVB_T2_1_7M_IF_5M:
			...
			//LPF
			if ((R912_INFO.R912_Standard == R912_DVB_T_8M_IF_5M) ||
...
R912_ErrCode R912_DVBS_Setting(struct r912_priv *priv, R912_Set_Info R912_INFO)
{
	...
	if (R912_INFO.R912_Standard != R912_pre_standard) {
		...
		if (R912_INFO.R912_DVBS_OutputSignal_Mode != SINGLEOUT) {
			...
			R912_Array[35] |=
				0x80; //R710 R11[4]    R912:R43[7]   43-8=35   43(0x2B) is addr ; [35] is data
		}
		...
		if (R912_INFO.R912_DVBS_AGC_Mode != AGC_POSITIVE) {
			...
			R912_Array[37] |=
				0x08; //R710 R13[4]    R912:R45[3]   45-8=37   45(0x2D) is addr ; [37] is data
		}
		...
		//Set GPO Low
		R912_I2C.RegAddr =
			0x17; // R912:R23[4:2]  23-8=15  23(0x17) is addr ; [15] is data
	...
	if (R912_PLL(priv, R912_INFO.RF_KHz, R912_INFO.R912_Standard) !=
	    RT_Success) {
		return RT_Fail;
	}
	...
	if ((R912_INFO.RF_KHz >= 1200000) && (R912_INFO.RF_KHz <= 1750000)) {
		R912_Array[23] = (R912_Array[23] & 0x00) |
				 0x93; //R912:R31[7:0]    1.24/0.64
	} else {
		R912_Array[23] = (R912_Array[23] & 0x00) |
				 0x83; //R912:R31[7:0]   1.14/0.64
	}
	...
	if ((R912_INFO.RF_KHz >= 1600000) && (R912_INFO.RF_KHz <= 1950000)) {
		R912_Array[35] |=
			0x20; //LNA Mode with att   //R710 R2[6]    R912:R43[5]   43-8=35   43(0x2B) is addr ; [35] is data
	} else {
	...
	if (R912_INFO.DVBS_BW != R912_pre_DVBS_bw) {
		if (R912_INFO.DVBS_BW > 67400) {
		...
		else if ((R912_INFO.DVBS_BW > 62360) &&
			 (R912_INFO.DVBS_BW <= 67400)) {
			Coarse = 31;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 38000) &&
			   (R912_INFO.DVBS_BW <= 62360)) {
			fine_tune = 1;
			...
			fine_tune = 0;
			   ...
			   (R912_INFO.DVBS_BW <= 8000)) {
			Coarse = 0;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 8000) &&
			   (R912_INFO.DVBS_BW <= 10000)) {
			Coarse = 1;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 10000) &&
			   (R912_INFO.DVBS_BW <= 12000)) {
			Coarse = 2;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 12000) &&
			   (R912_INFO.DVBS_BW <= 14200)) {
			Coarse = 3;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 14200) &&
			   (R912_INFO.DVBS_BW <= 16000)) {
			Coarse = 4;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 16000) &&
			   (R912_INFO.DVBS_BW <= 17800)) {
			Coarse = 5;
			fine_tune = 0;
		} else if ((R912_INFO.DVBS_BW > 17800) &&
			   (R912_INFO.DVBS_BW <= 18600)) {
			Coarse = 5;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 18600) &&
			   (R912_INFO.DVBS_BW <= 20200)) {
			Coarse = 6;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 20200) &&
			   (R912_INFO.DVBS_BW <= 22400)) {
			Coarse = 7;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 22400) &&
			   (R912_INFO.DVBS_BW <= 24600)) {
			Coarse = 9;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 24600) &&
			   (R912_INFO.DVBS_BW <= 25400)) {
			Coarse = 10;
			fine_tune = 0;
		} else if ((R912_INFO.DVBS_BW > 25400) &&
			   (R912_INFO.DVBS_BW <= 26000)) {
			Coarse = 10;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 26000) &&
			   (R912_INFO.DVBS_BW <= 27200)) {
			Coarse = 11;
			fine_tune = 0;
		} else if ((R912_INFO.DVBS_BW > 27200) &&
			   (R912_INFO.DVBS_BW <= 27800)) {
			Coarse = 11;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 27800) &&
			   (R912_INFO.DVBS_BW <= 30200)) {
			Coarse = 12;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 30200) &&
			   (R912_INFO.DVBS_BW <= 32600)) {
			Coarse = 13;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 32600) &&
			   (R912_INFO.DVBS_BW <= 33800)) {
			Coarse = 14;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 33800) &&
			   (R912_INFO.DVBS_BW <= 36800)) {
			Coarse = 15;
			fine_tune = 1;
		} else if ((R912_INFO.DVBS_BW > 36800) &&
			   (R912_INFO.DVBS_BW <= 38000)) {
			Coarse = 16;
			fine_tune = 1;
		}
		...
		R912_Array[39] =
			((R912_Array[39] | (fine_tune << 6)) | (Coarse));
...
R912_ErrCode R912_SetPllData(struct r912_priv *priv, R912_Set_Info R912_INFO)
{
	...
	if (R912_INFO.R912_Standard != R912_DVB_S) {
		if (R912_SetStandard(priv, R912_INFO.R912_Standard) !=
		    RT_Success) {
			return RT_Fail;
		}
...
R912_ErrCode R912_Standby(struct r912_priv *priv)
{
	// R912:R8[7]
	R912_Array[0] =
		(R912_Array[0] | 0x80); //LNA off   All / LNA / Buffer PW off
	//  R912:R37[3]
...
R912_ErrCode R912_GetRfGain(struct r912_priv *priv,
			    R912_RF_Gain_Info *pR912_rf_gain)
{
	R912_I2C_Len.RegAddr = 0x00;
	...
	pR912_rf_gain->RF_gain1 =
		(R912_I2C_Len.Data[4] & 0x1F); //lna    //848:4[4:0]
	pR912_rf_gain->RF_gain2 =
		((R912_I2C_Len.Data[3] & 0xF0) >> 4); //rf	 //848:3[4:0]
	pR912_rf_gain->RF_gain3 =
		(R912_I2C_Len.Data[3] & 0x0F); //mixer //848:3[4:0]
	/*
	Test_Len.Data[0]=0;
	Test_Len.Data[1]=0;
	...
	if (R912_SATELLITE_FLAG == 0) {
		...
		pR912_rf_gain->RF_gain_comb = (pR912_rf_gain->RF_gain1 * 14 +
					       pR912_rf_gain->RF_gain2 * 12 +
					       pR912_rf_gain->RF_gain3 * 12);
	} else {
		if (pR912_rf_gain->RF_gain1 <= 2) {
			pR912_rf_gain->RF_gain1 = 0;
		} else if (pR912_rf_gain->RF_gain1 > 2 &&
			   pR912_rf_gain->RF_gain1 <= 9) {
			pR912_rf_gain->RF_gain1 -= 2;
		} else if (pR912_rf_gain->RF_gain1 > 9 &&
			   pR912_rf_gain->RF_gain1 <= 12) {
			pR912_rf_gain->RF_gain1 = 8;
		} else if (pR912_rf_gain->RF_gain1 > 12 &&
			   pR912_rf_gain->RF_gain1 <= 20) {
			pR912_rf_gain->RF_gain1 -= 4;
		} else if (pR912_rf_gain->RF_gain1 > 20 &&
			   pR912_rf_gain->RF_gain1 <= 22) {
...
R912_ErrCode R912_RfGainMode(struct r912_priv *priv,
			     R912_RF_Gain_TYPE R912_RfGainType)
{
	u8 MixerGain = 0;
	u8 RfGain = 0;
	u8 LnaGain = 0;
	...
	if (R912_RfGainType == RF_MANUAL) {
		...
		RfGain = ((R912_I2C_Len.Data[3] & 0xF0) >>
			  4); //rf		 // 848:3[4:0]
		...
		//set Mixer gain
		R912_I2C.RegAddr = 0x0F;
		R912_Array[7] = (R912_Array[7] & 0xF0) |
...
//--------------------------------------------------------------------//
static R912_ErrCode R912_SetXtalIntCap(struct r912_priv *priv,
				       R912_Xtal_Cap_TYPE R912_XtalCapType)
{
...
static void r912_release(struct dvb_frontend *fe)
{
	kfree(fe->tuner_priv);
	fe->tuner_priv = NULL;
	return;
...
static int r912_set_params(struct dvb_frontend *fe)
{
	...
	default:
		...
		R912_INFO.R912_Standard = R912_DVB_S;
		R912_INFO.DVBS_BW = (c->symbol_rate / 200 * 135 + 2000000) /
				    1000 * 2; //unit KHz
...
#if 0
	if (ret) {
		pr_warn("R912: Tuner lock failed\n");
		goto exit;
	}
#endif
	for (i = 0; i < 5; i++) {
				 ...
				 struct i2c_adapter *i2c)
{
	...
	memcpy(&fe->ops.tuner_ops, &r912_tuner_ops,
	       sizeof(struct dvb_tuner_ops));

	fe->tuner_priv = priv;
	return fe;
}
EXPORT_SYMBOL_GPL(r912_attach);
