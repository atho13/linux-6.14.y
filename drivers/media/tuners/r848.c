...
static R848_Freq_Info_Type R848_Freq_Sel(u32 LO_freq, u32 RF_freq,
				  R848_Standard_Type R848_Standard)
{
	R848_Freq_Info_Type R848_Freq_Info;

	/* RF dependent parameter - LNA band R13[6:5] */
	if (RF_freq < R848_LNA_LOW_LOWEST[R848_TF_BEAD]) // <164
		R848_Freq_Info.LNA_BAND = 0x60; // ultra low - R848:R13[6:5]
	else if ((RF_freq >= R848_LNA_LOW_LOWEST[R848_TF_BEAD]) &&
		 (RF_freq < R848_LNA_MID_LOW[R848_TF_BEAD])) // 164~388
		R848_Freq_Info.LNA_BAND = 0x40; // low - R848:R13[6:5]
	else if ((RF_freq >= R848_LNA_MID_LOW[R848_TF_BEAD]) &&
		 (RF_freq < R848_LNA_HIGH_MID[R848_TF_BEAD])) // 388~612
		R848_Freq_Info.LNA_BAND = 0x20; // mid - R848:R13[6:5]
	else // >612
		R848_Freq_Info.LNA_BAND = 0x00; // high	- R848:R13[6:5]

	/* LO dependent parameter - IMR point */
	if (LO_freq < 133000)
	...
	if (LO_freq < 133000)
		...
		R848_Freq_Info.RF_POLY = 0x00; // high
	else
...
static int I2C_Write(struct r848_priv *priv, I2C_TYPE *R848_I2C)
{
...
static R848_ErrCode R848_Cal_Prepare(struct r848_priv *priv, u8 u1CalFlag)
{
	...
	priv->cfg->R848_Array[4] =
		(priv->cfg->R848_Array[4] & 0xDF) |
		Cal_Info.RFBUF_OUT; //  R848:R12[5]  12-8=4  12(0x0C) is addr ; [4] is data
	...
	/* RF_Buf Power */
	priv->cfg->R848_Array[1] = (priv->cfg->R848_Array[1] & 0xFB) |
				   Cal_Info.RFBUF_POWER;
	...
	priv->cfg->R848_Array[0] =
		(priv->cfg->R848_Array[0] & 0x7F) |
		Cal_Info.LNA_POWER; //  R848:R8[7]  8-8=0  8(0x08) is addr ; [0] is data
	...
	priv->cfg->R848_Array[6] =
		(priv->cfg->R848_Array[6] & 0x9F) |
		Cal_Info.TF_CAL; // R848:R14[6:5]  14-8=6  14(0x0E) is addr ; [6] is data
	ret = r848_wr(priv, 0x0e, priv->cfg->R848_Array[6]);
	if (ret)
		return ret;

	/* LNA gain */
	priv->cfg->R848_Array[5] =
		(priv->cfg->R848_Array[5] & 0x60) |
		Cal_Info.LNA_GAIN; // R848:R13[7:0]  13-8=5  13(0x0D) is addr ; [5] is data
	ret = r848_wr(priv, 0x0d, priv->cfg->R848_Array[5]);
	if (ret)
		return ret;

	/* Mixer Amp Gain */
	priv->cfg->R848_Array[7] =
		(priv->cfg->R848_Array[7] & 0xE0) |
		Cal_Info.MIXER_AMP_GAIN; // R848:R15[4:0]  15-8=7  15(0x0F) is addr ; [7] is data
	...
	priv->cfg->R848_Array[26] = (priv->cfg->R848_Array[26] & 0xE0) |
	...
	priv->cfg->R848_Array[30] = (priv->cfg->R848_Array[30] & 0xF7) |
				    Cal_Info.FILTER_6DB | 0x80;
	...
	priv->cfg->R848_Array[14] = priv->cfg->R848_Array[14] |
				    0x01; //force far-end mode
...
static R848_ErrCode R848_Xtal_Check(struct r848_priv *priv)
{
	...
	for (i = 0; i < XTAL_CHECK_SIZE; i++) {
		...
		if (i <= XTAL_SMALL_HIGHEST) {
			priv->cfg->R848_Array[15] =
				(priv->cfg->R848_Array[15] & 0x9F) |
				((u8)(XTAL_SMALL_HIGHEST - i) << 5);
			priv->cfg->R848_Array[19] =
				(priv->cfg->R848_Array[19] & 0xFE) |
				0x00; //| 0x00;  //SMALL Gm(0)
		} else if (i == XTAL_LARGE_HIGHEST) {
			//priv->cfg->R848_Array[15] =
			//	(priv->cfg->R848_Array[15] & 0x87) |
			//	0x00 | 0x18;  //3v Gm, highest
			priv->cfg->R848_Array[15] =
				(priv->cfg->R848_Array[15] & 0x9F) | 0X00;
			priv->cfg->R848_Array[19] =
				(priv->cfg->R848_Array[19] & 0xFE) |
		 ...
		 */
#if (R848_Xtal == 16000)
		if (((buf[2] & 0x40) == 0x40) && ((buf[2] & 0x3F) <= 11) &&
		    ((buf[2] & 0x3F) >= 5))
#else
		if (((buf[2] & 0x40) == 0x40) && ((buf[2] & 0x3F) <= 48) &&
		    ((buf[2] & 0x3F) >= 42))
#endif
		{
...
static R848_ErrCode R848_PLL(struct r848_priv *priv, u32 LO_Freq,
		      R848_Standard_Type R848_Standard)
{
	int ret;
	u8 buf[3];
	...
	/* Div Current R20[7:6]=2'b01(150uA) */
	...
	/* CP Offset R21[7] */
	priv->cfg->R848_Array[13] = (priv->cfg->R848_Array[13] & 0x7F) |
				    CP_OFFSET;
	...
	{
		/* Set XTAL Power */
		if ((priv->cfg->R848_Xtal_Pwr < XTAL_SMALL_HIGH) &&
		    (LO_Freq > (64000 + 8500))) {
			...
			if (priv->cfg->R848_Xtal_Pwr <= XTAL_SMALL_HIGHEST) {
				XTAL_POW1 = 0x00; // High, R16[4]=0 - R848:R23[7]
				XTAL_POW0 = ((u8)(XTAL_SMALL_HIGHEST -
						  priv->cfg->R848_Xtal_Pwr)
					     << 5); // R848:R23[6:5]
				XTAL_GM = 0x00; // SMALL(0), R27[0]=0
			} else if (priv->cfg->R848_Xtal_Pwr ==
				   XTAL_LARGE_HIGHEST) {
				XTAL_POW1 = 0x00; // High, R848:R23[7]
				XTAL_POW0 =
					0x00; // Highest, R848:R23[6:5]
	...
	priv->cfg->R848_Array[19] = (priv->cfg->R848_Array[19] & 0xFE) |
				    XTAL_GM;
	ret = r848_wr(priv, 0x1b, priv->cfg->R848_Array[19]);
	if (ret)
		return ret;
	...
	priv->cfg->R848_Array[15] = (priv->cfg->R848_Array[15] & 0x9F) |
				    XTAL_POW0;
	...
	priv->cfg->R848_Array[15] = (priv->cfg->R848_Array[15] & 0x7F) |
				    XTAL_POW1;
	//	R848_I2C.Data    = priv->cfg->R848_Array[15];
	//	if(I2C_Write(priv,&R848_I2C) != RT_Success)
	//	    return RT_Fail;
	...
	/* IQ Gen ON */
	priv->cfg->R848_Array[31] = (priv->cfg->R848_Array[31] & 0xFD) |
				    0x00;
	...
	priv->cfg->R848_Array[19] = (priv->cfg->R848_Array[19] & 0xE7) |
				    SDM_RES;
	ret = r848_wr(priv, 0x1b, priv->cfg->R848_Array[19]);
	if (ret)
		return ret;
	...
	/* Xtal Div */
	if (R848_Standard == R848_STD_SIZE) {
		/* For Cal */
		R848_XtalDiv = XTAL_DIV1;
		priv->cfg->R848_Array[16] = priv->cfg->R848_Array[16] &
					    0xFB; //b2=0  // R848:R24[2]
		PLL_Ref = R848_Xtal;
	} else {
		/* DTV_Standard */
		...
		if (u1XtalDivRemain == 1) {
			...
			/* div2, note that agc clk also div2 */
	...
	/* Si */
	priv->cfg->R848_Array[12] = (priv->cfg->R848_Array[12] & 0xCF) |
				    ((Si << 4));
	ret = r848_wr(priv, 0x14, priv->cfg->R848_Array[12]);
	if (ret)
		return ret;
	...
	for (VCO_current_trial = 0; VCO_current_trial < 3;
		...
		if ((buf[2] & 0x40) == 0x00) {
			...
			priv->cfg->R848_Array[18] =
				(priv->cfg->R848_Array[18] & 0x1F) |
				((2 - VCO_current_trial)
	...
	if (VCO_current_trial == 2) {
		...
		if ((buf[2] & 0x40) == 0x00) {
			...
			priv->cfg->R848_Array[19] =
				(priv->cfg->R848_Array[19] & 0xFE) | XTAL_GM;
			ret = r848_wr(priv, 0x1b, priv->cfg->R848_Array[19]);
			if (ret)
				return ret;
		}
	...
	priv->cfg->R848_Array[14] = (priv->cfg->R848_Array[14] & 0xFE) |
				    u1RfFlag;
	//	R848_I2C.Data = priv->cfg->R848_Array[14];
	//	if(I2C_Write(priv,&R848_I2C) != RT_Success)
	//	    return RT_Fail;
	...
	priv->cfg->R848_Array[30] = (priv->cfg->R848_Array[30] & 0x7F) |
				    u1PulseFlag;
	//	R848_I2C.Data = priv->cfg->R848_Array[30];
	//	if(I2C_Write(priv,&R848_I2C) != RT_Success)
	//	    return RT_Fail;
	...
	priv->cfg->R848_Array[32] = (priv->cfg->R848_Array[32] & 0xF7) |
				    u1SPulseFlag;
	//    R848_I2C.Data = priv->cfg->R848_Array[32];
	//    if(I2C_Write(priv,&R848_I2C) != RT_Success)
	//	   return RT_Fail;
...
static R848_ErrCode R848_MUX(struct r848_priv *priv, u32 LO_KHz, u32 RF_KHz,
		      R848_Standard_Type R848_Standard)
{
	int ret;
	...
	priv->cfg->R848_Array[5] = (priv->cfg->R848_Array[5] & 0x9F) |
				   Freq_Info1.LNA_BAND;
	...
	priv->cfg->R848_Array[25] = (priv->cfg->R848_Array[25] & 0x3F) |
				    Freq_Info1.RF_POLY;
	...
	priv->cfg->R848_Array[1] = (priv->cfg->R848_Array[1] & 0x07) |
				   (Freq_Info1.LPF_CAP << 3);
	//	R848_I2C.Data = priv->cfg->R848_Array[1];
	//	if(I2C_Write(priv,&R848_I2C) != RT_Success)
	//		return RT_Fail;
	ret = r848_wr(priv, 0x09, priv->cfg->R848_Array[1]);
	...
	priv->cfg->R848_Array[2] = (priv->cfg->R848_Array[2] & 0xE0) |
				   (Freq_Info1.LPF_NOTCH);
...
// output: TRUE or FALSE
//-----------------------------------------------------------------------------------/
static R848_ErrCode R848_CompreCor(struct r848_priv *priv, R848_SectType *CorArry)
{
	u8 CompCunt = 0;
...
/* Purpose: record IMC results by input gain/phase location then
 *          adjust gain or phase positive 1 step and negtive 1
 *          step, both record results
...
static R848_ErrCode R848_IQ_Tree(struct r848_priv *priv, u8 FixPot, u8 FlucPot,
			  u8 PotReg, R848_SectType *CompareTree)
{
	int ret;
	u8 TreeCunt = 0;
	u8 TreeTimes = 3;
	...
	for (TreeCunt = 0; TreeCunt < TreeTimes; TreeCunt++) {
		...
		if (R848_Muti_Read(priv, &CompareTree[TreeCunt].Value) !=
		    RT_Success)
			return RT_Fail;
...
static R848_ErrCode R848_Section(struct r848_priv *priv, R848_SectType *IQ_Pont)
{
	...
	if (R848_IQ_Tree(priv, Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y,
...
static R848_ErrCode R848_IMR_Cross(struct r848_priv *priv, R848_SectType *IQ_Pont,
			    u8 *X_Direct)
{
	int ret;

	/* (0,0)(0,Q-1)(0,I-1)(Q-1,0)(I-1,0)+(0,Q-2)(0,I-2)(Q-2,0)(I-2,0) */
	R848_SectType Compare_Cross[9];
	...
	for (CrossCount = 0; CrossCount < 9; CrossCount++) {
		...
		if (R848_Muti_Read(priv, &Compare_Cross[CrossCount].Value) !=
		    RT_Success)
			return RT_Fail;
...
// output: TRUE or FALSE
//-------------------------------------------------------------------------------------//
static R848_ErrCode R848_CompreStep(struct r848_priv *priv, R848_SectType *StepArry,
			     u8 Pace)
{
	int ret;
	...
	while (((StepTemp.Gain_X & 0x1F) < R848_IMR_TRIAL) &&
	       ((StepTemp.Phase_Y & 0x1F) < R848_IMR_TRIAL)) {
		if (Pace == 0x10)
			StepTemp.Gain_X++;
		else
			StepTemp.Phase_Y++;
...
static R848_ErrCode R848_IMR_Iqcap(struct r848_priv *priv, R848_SectType *IQ_Point)
{
	R848_SectType Compare_Temp;
	int i = 0, ret;
...
static R848_ErrCode R848_IQ(struct r848_priv *priv, R848_SectType *IQ_Pont)
{
	int ret;
	R848_SectType Compare_IQ[3];
	u8 VGA_Count = 0;
	...
	for (VGA_Count = 11; VGA_Count < 16; VGA_Count++) {
		...
		//		if(I2C_Write(priv,&R848_I2C) != RT_Success)
		//			return RT_Fail;
		ret = r848_wr(priv, 0x14,
			      (priv->cfg->R848_Array[12] & 0xF0) + VGA_Count);
		if (ret)
			return ret;
	...
	if (X_Direction == 1) {
		...
		if (R848_CompreStep(priv, &Compare_IQ[0], 0x10) !=
		    RT_Success) //X
			return RT_Fail;
	} else {
		//compare and find min of 3 points. determine I/Q direction
		...
		//increase step to find min value of this direction
		if (R848_CompreStep(priv, &Compare_IQ[0], 0x11) !=
		    RT_Success) //Y
			return RT_Fail;
	}
	...
	//Another direction
	if (X_Direction == 1) {
		if (R848_IQ_Tree(priv, Compare_IQ[0].Gain_X,
		...
		if (R848_IQ_Tree(priv, Compare_IQ[0].Phase_Y,
...
// output: TRUE or FALSE
//----------------------------------------------------------------------------------------//
static R848_ErrCode R848_F_IMR(struct r848_priv *priv, R848_SectType *IQ_Pont)
{
	int ret;
	R848_SectType Compare_IQ[3];
	R848_SectType Compare_Bet[3];
...
static R848_ErrCode R848_SetTF(struct r848_priv *priv, u32 u4FreqKHz, u8 u1TfType)
{
	int ret;
	u8 u1FreqCount = 0;
	u32 u4Freq1 = 0;
	...
	if (u4FreqKHz < R848_LNA_LOW_LOWEST[R848_TF_BEAD]) //Ultra Low
	{
		...
		while ((u4FreqKHz <
			R848_TF_Freq_Lowest[u1TfType][u1FreqCount]) &&
		       (u1FreqCount < R848_TF_LOWEST_NUM)) {
			u1FreqCount++;
		}
		...
		if (u1FreqCount == 0) {
			...
			R848_TF = R848_TF_Result_Lowest[u1TfType]
						       [R848_TF_LOWEST_NUM - 1];
		} else {
			u1TF_Set_Result1 =
				R848_TF_Result_Lowest[u1TfType][u1FreqCount - 1];
			u1TF_Set_Result2 =
				R848_TF_Result_Lowest[u1TfType][u1FreqCount];
			u4Freq1 =
				R848_TF_Freq_Lowest[u1TfType][u1FreqCount - 1];
	...
	} else if ((u4FreqKHz >= R848_LNA_LOW_LOWEST[R848_TF_BEAD]) &&
		   (u4FreqKHz < R848_LNA_MID_LOW[R848_TF_BEAD])) //Low
	{
		u1TFCalNum = R848_TF_LOW_NUM;
		while ((u4FreqKHz < R848_TF_Freq_Low[u1TfType][u1FreqCount]) &&
		       (u1FreqCount < R848_TF_LOW_NUM)) {
			u1FreqCount++;
		}
		...
		if (u1FreqCount == 0) {
			...
			R848_TF = R848_TF_Result_Low[u1TfType]
						    [R848_TF_LOW_NUM - 1];
		} else {
			u1TF_Set_Result1 =
				R848_TF_Result_Low[u1TfType][u1FreqCount - 1];
			u1TF_Set_Result2 =
				R848_TF_Result_Low[u1TfType][u1FreqCount];
	...
	} else if ((u4FreqKHz >= R848_LNA_MID_LOW[R848_TF_BEAD]) &&
		   (u4FreqKHz < R848_LNA_HIGH_MID[R848_TF_BEAD])) //Mid
	{
		u1TFCalNum = R848_TF_MID_NUM;
		while ((u4FreqKHz < R848_TF_Freq_Mid[u1TfType][u1FreqCount]) &&
		       (u1FreqCount < R848_TF_MID_NUM)) {
			u1FreqCount++;
		}
		...
		if (u1FreqCount == 0) {
			...
			R848_TF = R848_TF_Result_Mid[u1TfType]
						    [R848_TF_MID_NUM - 1];
		} else {
			u1TF_Set_Result1 =
				R848_TF_Result_Mid[u1TfType][u1FreqCount - 1];
			u1TF_Set_Result2 =
				R848_TF_Result_Mid[u1TfType][u1FreqCount];
			u4Freq1 = R848_TF_Freq_Mid[u1TfType][u1FreqCount - 1];
			u4Freq2 = R848_TF_Freq_Mid[u1TfType][u1FreqCount];
			u4Ratio = (u4Freq1 - u4FreqKHz) * 100 /
				  (u4Freq1 - u4Freq2);
			R848_TF = u1TF_Set_Result1 +
	...
	} else //HIGH
	{
		...
		while ((u4FreqKHz < R848_TF_Freq_High[u1TfType][u1FreqCount]) &&
		       (u1FreqCount < R848_TF_HIGH_NUM)) {
			u1FreqCount++;
		}
		...
		if (u1FreqCount == 0) {
			...
			R848_TF = R848_TF_Result_High[u1TfType]
						     [R848_TF_HIGH_NUM - 1];
		} else {
			u1TF_Set_Result1 =
				R848_TF_Result_High[u1TfType][u1FreqCount - 1];
			u1TF_Set_Result2 =
				R848_TF_Result_High[u1TfType][u1FreqCount];
...
static R848_ErrCode R848_IMR(struct r848_priv *priv, u8 IMR_MEM, bool IM_Flag)
{
	u32 RingVCO = 0;
	u32 RingFreq = 0;
	u8 u1MixerGain = 8;
	...
	RingVCO = 3200000;
	priv->cfg->R848_Array[31] &=
		0x3F; //clear ring_div1, R24[7:6]	//R848:R39[7:6]  39-8=31  39(0x27) is addr ; [31] is data
	priv->cfg->R848_Array[25] &=
		0xFC; //clear ring_div2, R25[1:0]	//R848:R33[1:0]  33-8=25  33(0x21) is addr ; [25] is data
	priv->cfg->R848_Array[25] &=
		0xDF; //clear vco_band, R25[5]		//R848:R33[5]    33-8=25  33(0x21) is addr ; [25] is data
	priv->cfg->R848_Array[31] &=
		0xC3; //clear ring_div_num, R24[3:0]//R848:R39[5:2]  39-8=31  39(0x27) is addr ; [31] is data
	...
	priv->cfg->R848_Array[7] = (priv->cfg->R848_Array[7] & 0xE0) |
				   u1MixerGain;
	R848_I2C.Data = priv->cfg->R848_Array[7];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	if (((RingFreq < R848_RING_POWER_FREQ_LOW) ||
	     (RingFreq > R848_RING_POWER_FREQ_HIGH))) //R848:R33[3:2]
		priv->cfg->R848_Array[25] = (priv->cfg->R848_Array[25] & 0xF3) |
	...
	if (R848_MUX(priv, RingFreq - R848_IMR_IF, RingFreq, R848_STD_SIZE) !=
	    RT_Success) //IMR MUX (LO, RF)
		return RT_Fail;

	if (R848_PLL(priv, (RingFreq - R848_IMR_IF), R848_STD_SIZE) !=
	    RT_Success) //IMR PLL
		return RT_Fail;
			  ...
			  R848_Standard_Type R848_Standard)
{
	...
	if (R848_Standard != R848_DVB_S) {
		for (InitArrayCunt = 0; InitArrayCunt < R848_REG_NUM;
		     InitArrayCunt++) {
			//R848_I2C_Len.Data[InitArrayCunt] = R848_iniArray_hybrid[InitArrayCunt];
			buf[InitArrayCunt + 1] =
				R848_iniArray_hybrid[InitArrayCunt];
			priv->cfg->R848_Array[InitArrayCunt] =
				R848_iniArray_hybrid[InitArrayCunt];
		}
	} else {
		for (InitArrayCunt = 0; InitArrayCunt < R848_REG_NUM;
			...
			buf[InitArrayCunt + 1] =
				R848_iniArray_dvbs[InitArrayCunt];
			priv->cfg->R848_Array[InitArrayCunt] =
				R848_iniArray_dvbs[InitArrayCunt];
		}
	}
...
static u8 R848_Filt_Cal_ADC(struct r848_priv *priv, u32 IF_Freq, u8 R848_BW,
		     u8 FilCal_Gap)
{
	...
	//Must do before PLL()
	if (R848_MUX(priv, u4RingFreq + IF_Freq, u4RingFreq, R848_STD_SIZE) !=
	    RT_Success) //FilCal MUX (LO_Freq, RF_Freq)
		return RT_Fail;

	//Set PLL
	if (R848_PLL(priv, (u4RingFreq + IF_Freq), R848_STD_SIZE) !=
	...
	priv->cfg->R848_Array[10] = (priv->cfg->R848_Array[10] & 0xF0) |
				    0; //filter code=0
	R848_I2C.Data = priv->cfg->R848_Array[10];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
...
static R848_ErrCode R848_DVBS_Setting(struct r848_priv *priv, R848_Set_Info R848_INFO)
{
	u32 LO_KHz;
	...
	if ((R848_INFO.RF_KHz >= 1200000) && (R848_INFO.RF_KHz <= 1750000)) {
		priv->cfg->R848_Array[23] = (priv->cfg->R848_Array[23] & 0x00) |
					    0x93; //R848:R31[7:0]    1.24/0.64
	} else {
		priv->cfg->R848_Array[23] = (priv->cfg->R848_Array[23] & 0x00) |
					    0x83; //R848:R31[7:0]   1.14/0.64
	}
	...
	if (R848_INFO.RF_KHz >= 2000000) {
		...
		priv->cfg->R848_Array[38] = (priv->cfg->R848_Array[38] & 0xCF) |
					    0x20; //R848:R46[4:5]
	...
	if ((R848_INFO.RF_KHz >= 1600000) && (R848_INFO.RF_KHz <= 1950000)) {
		priv->cfg->R848_Array[35] |=
			0x20; //LNA Mode with att   //R710 R2[6]    R848:R43[5]   43-8=35   43(0x2B) is addr ; [35] is data
	...
	if (R848_INFO.R848_DVBS_OutputSignal_Mode != SINGLEOUT) {
		...
		priv->cfg->R848_Array[35] |=
			0x80; //R710 R11[4]    R848:R43[7]   43-8=35   43(0x2B) is addr ; [35] is data
	}
	...
	if (R848_INFO.R848_DVBS_AGC_Mode != AGC_POSITIVE) {
		...
		priv->cfg->R848_Array[37] |=
			0x08; //R710 R13[4]    R848:R45[3]   45-8=37   45(0x2D) is addr ; [37] is data
	}
	...
	if (R848_INFO.DVBS_BW > 67400) {
		...
		Coarse = 31;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 38000) &&
		   (R848_INFO.DVBS_BW <= 62360)) {
		fine_tune = 1;
		...
		Coarse = 1;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 10000) &&
		   (R848_INFO.DVBS_BW <= 12000)) {
		Coarse = 2;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 12000) &&
		   (R848_INFO.DVBS_BW <= 14200)) {
		Coarse = 3;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 14200) &&
		   (R848_INFO.DVBS_BW <= 16000)) {
		Coarse = 4;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 16000) &&
		   (R848_INFO.DVBS_BW <= 17800)) {
		Coarse = 5;
		fine_tune = 0;
	} else if ((R848_INFO.DVBS_BW > 17800) &&
		   (R848_INFO.DVBS_BW <= 18600)) {
		Coarse = 5;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 18600) &&
		   (R848_INFO.DVBS_BW <= 20200)) {
		Coarse = 6;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 20200) &&
		   (R848_INFO.DVBS_BW <= 22400)) {
		Coarse = 7;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 22400) &&
		   (R848_INFO.DVBS_BW <= 24600)) {
		Coarse = 9;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 24600) &&
		   (R848_INFO.DVBS_BW <= 25400)) {
		Coarse = 10;
		fine_tune = 0;
	} else if ((R848_INFO.DVBS_BW > 25400) &&
		   (R848_INFO.DVBS_BW <= 26000)) {
		Coarse = 10;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 26000) &&
		   (R848_INFO.DVBS_BW <= 27200)) {
		Coarse = 11;
		fine_tune = 0;
	} else if ((R848_INFO.DVBS_BW > 27200) &&
		   (R848_INFO.DVBS_BW <= 27800)) {
		Coarse = 11;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 27800) &&
		   (R848_INFO.DVBS_BW <= 30200)) {
		Coarse = 12;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 30200) &&
		   (R848_INFO.DVBS_BW <= 32600)) {
		Coarse = 13;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 32600) &&
		   (R848_INFO.DVBS_BW <= 33800)) {
		Coarse = 14;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 33800) &&
		   (R848_INFO.DVBS_BW <= 36800)) {
		Coarse = 15;
		fine_tune = 1;
	} else if ((R848_INFO.DVBS_BW > 36800) &&
		   (R848_INFO.DVBS_BW <= 38000)) {
		Coarse = 16;
		fine_tune = 1;
	}
	...
	priv->cfg->R848_Array[39] =
		((priv->cfg->R848_Array[39] | (fine_tune << 6)) |
		 (Coarse_tune));
	...
	//Set GPO Low
	R848_I2C.RegAddr =
		0x17; // R848:R23[4:2]  23-8=15  23(0x17) is addr ; [15] is data
...
static R848_Sys_Info_Type R848_Sys_Sel(struct r848_priv *priv,
				R848_Standard_Type R848_Standard)
{
	...
	case R848_DTMB_6M_BW_IF_5M:
		...
		R848_Sys_Info.FILT_EXT_WIDEST = 0x00; //R38[2]=0, ext normal
		R848_Sys_Info.FILT_EXT_POINT =
			0x00; //R38[1:0]=0, lna=max-1 & Buf 4, hpf+1
		break;
	...
	case R848_DTMB_6M_BW_IF_4500:
		...
		R848_Sys_Info.FILT_EXT_WIDEST = 0x00; //R38[2]=0, ext normal
		R848_Sys_Info.FILT_EXT_POINT =
			0x02; //R38[1:0]=10, lna=max-1 & Buf 4, hpf+3
		break;

	case R848_ATSC:
		R848_Sys_Info.IF_KHz = 5070;
		R848_Sys_Info.BW = 0x40; //BW=7M 	 R848:R19[6:5]
		R848_Sys_Info.FILT_CAL_IF =
			7900; //CAL IF	  20130621 Ryan Modify
		R848_Sys_Info.HPF_COR = 0x04; //R19[3:0]=4
		R848_Sys_Info.FILT_EXT_ENA = 0x00; //R19[4]=0, ext disable
		R848_Sys_Info.FILT_EXT_WIDEST = 0x00; //R38[2]=0, ext normal
		R848_Sys_Info.FILT_EXT_POINT = 0x00; //R38[1:0]=0, lna=max-1
	...
	if ((R848_Standard == R848_DVB_T2_1_7M) || (R848_Standard == R848_FM))
		...
		R848_Sys_Info.V17M =
			0x00; //disable, (include DVBT2 1.7M IF=5MHz) R19[7]
...
static R848_SysFreq_Info_Type R848_SysFreq_Sel(struct r848_priv *priv,
					R848_Standard_Type R848_Standard,
					u32 RF_freq)
{
	...
	case R848_DVB_T2_1_7M_IF_5M:
		if ((RF_freq >= 300000) && (RF_freq <= 472000)) {
			R848_SysFreq_Info.LNA_VTH_L =
				0xA4; // LNA VTH/L=1.34/0.74     (R31=0xA4)
			R848_SysFreq_Info.MIXER_TOP =
				0x05; // MIXER TOP=10        (R36[3:0]=4'b0101)
			R848_SysFreq_Info.MIXER_VTH_L =
				0x95; // MIXER VTH/L=1.24/0.84  (R32=0x95)
			R848_SysFreq_Info.NRB_TOP =
		...
		R848_SysFreq_Info.LNA_TOP =
			0x03; // LNA TOP=4           (R35[2:0]=3'b011)
		R848_SysFreq_Info.RF_TOP =
			0x40; // RF TOP=5                        (R34[7:5]=3'b010)
		R848_SysFreq_Info.NRB_BW =
	...
	case R848_J83B_IF_5M:
		if (RF_freq < 165000) {
			...
			R848_SysFreq_Info.NRB_TOP =
				0x90; // Nrb TOP=6                       (R36[7:4]=4'b1001)   //R848:R36[7:4]
	...
	R848_SysFreq_Info.RF_FAST_DISCHARGE =
		0x00; //0 	 R848:R46[3:1]=3'b000
	R848_SysFreq_Info.RF_SLOW_DISCHARGE = 0x80; //4   R848:R22[7:5]=2'b100
	R848_SysFreq_Info.RFPD_PLUSE_ENA = 0x10; //1   R848:R38[4]=1

	R848_SysFreq_Info.LNA_FAST_DISCHARGE =
		0x0A; //10  R848:R43[4:0]=5'b01010
	...
	case R848_DVB_T2_1_7M_IF_5M:
		if (RF_freq >= 662000 && RF_freq <= 670000) {
			R848_SysFreq_Info.RF_SLOW_DISCHARGE =
				0x20; //1   R848:R22[7:5]=2'b001
			R848_SysFreq_Info.AGC_CLK = 0x18; //60Hz   R26[4:2]
		} else {
			R848_SysFreq_Info.RF_SLOW_DISCHARGE =
				0x80; //4   R848:R22[7:5]=2'b100
...
static R848_ErrCode R848_TF_Check(struct r848_priv *priv)
{
	...
	R848_I2C.RegAddr = 0x27;
	priv->cfg->R848_Array[31] = (priv->cfg->R848_Array[31] & 0x03) | 0x80 |
	...
	priv->cfg->R848_Array[25] =
		...
		0x8B; //out div=8, RF poly=low band, power=min_lp
	if (RingVCO >= 3200000)
		priv->cfg->R848_Array[25] = (priv->cfg->R848_Array[25] &
					     0xDF); //vco_band=high, R25[5]=0
	else
		priv->cfg->R848_Array[25] = (priv->cfg->R848_Array[25] |
					     0x20); //vco_band=low, R25[5]=1
	R848_I2C.RegAddr = 0x21;
	R848_I2C.Data = priv->cfg->R848_Array[25];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	//Must do before PLL()
	if (R848_MUX(priv, RingFreq + 5000, RingFreq, R848_STD_SIZE) !=
	    RT_Success) //FilCal MUX (LO_Freq, RF_Freq)
		return RT_Fail;

	//Set PLL
	if (R848_PLL(priv, (RingFreq + 5000), R848_STD_SIZE) !=
	    RT_Success) //FilCal PLL
		return RT_Fail;
...
static R848_ErrCode R848_SetStandard(struct r848_priv *priv,
			      R848_Standard_Type RT_Standard)
{
	I2C_TYPE R848_I2C;
	int i = 0;
	...
	if (R848_Fil_Cal_flag[RT_Standard] == 1) {
		R848_Fil_Cal_code[RT_Standard] = R848_Filt_Cal_ADC(
			priv, priv->cfg->Sys_Info1.FILT_CAL_IF,
			priv->cfg->Sys_Info1.BW, u1FilCalGap);
	...
	priv->cfg->R848_Array[11] = (priv->cfg->R848_Array[11] & 0xEF) |
				    priv->cfg->Sys_Info1.FILT_EXT_ENA;
	R848_I2C.Data = priv->cfg->R848_Array[11];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	if (priv->cfg->Sys_Info1.FILT_EXT_ENA == 0x10) //(%)
	{
		...
		if ((priv->cfg->Sys_Info1.FILT_EXT_POINT & 0x02) ==
		    0x02) //HPF+3
		{
	...
	priv->cfg->R848_Array[11] = (priv->cfg->R848_Array[11] & 0x9F) |
				    R848_Fil_Cal_BW[RT_Standard];
	R848_I2C.Data = priv->cfg->R848_Array[11];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	priv->cfg->R848_Array[11] = (priv->cfg->R848_Array[11] & 0x70) |
				    priv->cfg->Sys_Info1.HPF_COR |
				    priv->cfg->Sys_Info1.V17M;
	R848_I2C.Data = priv->cfg->R848_Array[11];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	priv->cfg->R848_Array[3] = (priv->cfg->R848_Array[3] & 0xBF) |
				   priv->cfg->Sys_Info1.TF_CUR;
	R848_I2C.Data = priv->cfg->R848_Array[3];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	priv->cfg->R848_Array[10] = (priv->cfg->R848_Array[10] & 0x9F) |
				    priv->cfg->Sys_Info1.FILT_CUR;
	...
	priv->cfg->R848_Array[4] = (priv->cfg->R848_Array[4] & 0xFB) |
				   priv->cfg->Sys_Info1.SWBUF_CUR;
	R848_I2C.Data = priv->cfg->R848_Array[4];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	priv->cfg->R848_Array[30] =
		(priv->cfg->R848_Array[30] & 0xF8) | 0x04 |
		priv->cfg->Sys_Info1.FILT_EXT_POINT; //ext both HPF/LPF
	R848_I2C.Data = priv->cfg->R848_Array[30];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	/*
...
*/
	...
	priv->cfg->R848_Array[18] = (priv->cfg->R848_Array[18] & 0xFC) |
				    priv->cfg->Sys_Info1.SWCAP_CLK;
	R848_I2C.Data = priv->cfg->R848_Array[18];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
...
static R848_ErrCode R848_SetFrequency(struct r848_priv *priv, R848_Set_Info R848_INFO)
{
	u32 LO_KHz;
	I2C_TYPE R848_I2C;
	...
	if (R848_INFO.R848_Standard != R848_DVB_S) {
		...
		if ((R848_INFO.RF_KHz < 40000) ||
		    (R848_INFO.RF_KHz > 1002000)) {
			return RT_Fail;
		}
	} else {
		// Check Input Frequency Range
		if ((R848_INFO.RF_KHz < 40000) ||
		    (R848_INFO.RF_KHz > 2400000)) {
			return RT_Fail;
		}
	}
	...
	if (R848_MUX(priv, LO_KHz, R848_INFO.RF_KHz, R848_INFO.R848_Standard) !=
	    RT_Success) //normal MUX
		return RT_Fail;

	//Set PLL
	if (R848_PLL(priv, LO_KHz, R848_INFO.R848_Standard) !=
	    RT_Success) //noraml PLL
		return RT_Fail;

	//Set TF
	if (R848_SetTF(priv, R848_INFO.RF_KHz, priv->cfg->R848_SetTfType) !=
	    RT_Success)
		return RT_Fail;
	...
	//medQctrl 1.5K
	if ((R848_INFO.RF_KHz >= 300000) &&
	    (R848_INFO.RF_KHz <= 472000)) //<473MHz and >299MHz
	...
	//3~6 shrink
	if ((R848_INFO.RF_KHz >= 300000) &&
	    (R848_INFO.RF_KHz <= 550000)) //<551MHz and >299MHz
	...
	SysFreq_Info1 = R848_SysFreq_Sel(priv, R848_INFO.R848_Standard,
					 R848_INFO.RF_KHz);

	// Set LNA_TOP
	R848_I2C.RegAddr = 0x23; //  R848:R35[2:0]
	priv->cfg->R848_Array[27] = (priv->cfg->R848_Array[27] & 0xF8) |
				    (SysFreq_Info1.LNA_TOP);
	...
	priv->cfg->R848_Array[28] = (priv->cfg->R848_Array[28] & 0xF0) |
				    (SysFreq_Info1.MIXER_TOP);
	...
	priv->cfg->R848_Array[24] = (priv->cfg->R848_Array[24] & 0x00) |
				    SysFreq_Info1.MIXER_VTH_L;
	...
	priv->cfg->R848_Array[26] = (priv->cfg->R848_Array[26] & 0x1F) |
				    SysFreq_Info1.RF_TOP;
	R848_I2C.Data = priv->cfg->R848_Array[26];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	priv->cfg->R848_Array[28] = (priv->cfg->R848_Array[28] & 0x0F) |
				    SysFreq_Info1.NRB_TOP;
	R848_I2C.Data = priv->cfg->R848_Array[28];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	priv->cfg->R848_Array[27] = (priv->cfg->R848_Array[27] & 0x3F) |
				    SysFreq_Info1.NRB_BW;
	R848_I2C.Data = priv->cfg->R848_Array[27];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	priv->cfg->R848_Array[4] = (priv->cfg->R848_Array[4] & 0xBF) |
				   SysFreq_Info1.BYP_LPF;
	R848_I2C.Data = priv->cfg->R848_Array[4];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;

	R848_I2C.RegAddr = 0x2E; //0 	 R848:R46[3:1]=0'b000
	priv->cfg->R848_Array[38] = (priv->cfg->R848_Array[38] & 0xF1) |
	...
	priv->cfg->R848_Array[14] = (priv->cfg->R848_Array[14] & 0x1F) |
	...
	priv->cfg->R848_Array[30] = (priv->cfg->R848_Array[30] & 0xEF) |
				    SysFreq_Info1.RFPD_PLUSE_ENA;
	R848_I2C.Data = priv->cfg->R848_Array[30];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;

	R848_I2C.RegAddr = 0x2B; //10  R848:R43[4:0]=5'b01010
	priv->cfg->R848_Array[35] = (priv->cfg->R848_Array[35] & 0xE0) |
	...
	priv->cfg->R848_Array[14] = (priv->cfg->R848_Array[14] & 0xE3) |
	...
	priv->cfg->R848_Array[9] = (priv->cfg->R848_Array[9] & 0x7F) |
				   SysFreq_Info1.LNAPD_PLUSE_ENA;
	R848_I2C.Data = priv->cfg->R848_Array[9];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	// Set AGC clk
	R848_I2C.RegAddr = 0x1A; //  R848:R26[4:2]
	priv->cfg->R848_Array[18] = (priv->cfg->R848_Array[18] & 0xE3) |
				    SysFreq_Info1.AGC_CLK;
	R848_I2C.Data = priv->cfg->R848_Array[18];
	if (I2C_Write(priv, &R848_I2C) != RT_Success)
		return RT_Fail;
	...
	priv->cfg->R848_Array[17] =
		(priv->cfg->R848_Array[17] | 0x80); //no clk out // R848:R25[7]
	...
	case R848_DVB_T2_1_7M_IF_5M:
		...
		priv->cfg->R848_Array[3] =
			(priv->cfg->R848_Array[3] | 0x80); //[7]=1
		R848_I2C.Data = priv->cfg->R848_Array[3];
		if (I2C_Write(priv, &R848_I2C) != RT_Success)
			return RT_Fail;

		R848_I2C.RegAddr = 0x0A; // R848:R10[5]
		priv->cfg->R848_Array[2] =
			(priv->cfg->R848_Array[2] & 0xDF); //[5]=0
		R848_I2C.Data = priv->cfg->R848_Array[2];
		if (I2C_Write(priv, &R848_I2C) != RT_Success)
			return RT_Fail;
...
static int r848_init(struct dvb_frontend *fe)
{
	...
	if (R848_InitReg(priv, R848_STD_SIZE) !=
...
static int r848_set_params(struct dvb_frontend *fe)
{
	...
	case SYS_DVBC_ANNEX_C:
		...
		/* set pll data */
		if (R848_SetStandard(priv, R848_INFO.R848_Standard) !=
		    RT_Success) {
			return RT_Fail;
		}
	...
	case SYS_DVBS2:
		...
		R848_INFO.DVBS_BW = (c->symbol_rate / 200 * 135 + 2000000) /
				    1000 * 2; //unit KHz
