...
#define BLINDSCAN_NONEDO		0x80  /* do not blind scan */
...
/* Get the TS input of the frontend */
...
struct sound_status_s {
	...
	unsigned short sound_mode; /* STEREO/DUAL/MONO/SAP */
	union {
		void *resrvred;
		__u64 reserved1;
	};
...
struct dvbsx_singlecable_parameters {
	...
	__u32 bank;
	/*
	 * Uncommitted switches setting for SCD2 (Similar to DiSEqC
	 * WriteN1 command, but lower 4 bits only)
	...
	__u32 uncommitted;
	/*
	 * Committed switches setting for SCD2 (Similar to DiSEqC
	 * WriteN0 command, but lower 4 bits only)
